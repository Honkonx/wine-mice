...
static int shm_unlink(const char *name) {
  ...
  namelen = strlen(name);
  fname = (char *)alloca(sizeof("/data/data/com.micewine.emu/files/usr/tmp/") -
                         1 + namelen + 1);
  memcpy(fname, "/data/data/com.micewine.emu/files/usr/tmp/",
         sizeof("/data/data/com.micewine.emu/files/usr/tmp/") - 1);
  memcpy(fname + sizeof("/data/data/com.micewine.emu/files/usr/tmp/") - 1, name,
         namelen + 1);
...
static void *get_shm(unsigned int idx) {
  ...
  if (entry >= shm_addrs_size) {
    ...
    memset(shm_addrs + shm_addrs_size, 0,
           (new_size - shm_addrs_size) * sizeof(shm_addrs[0]));
    shm_addrs_size = new_size;
  }

  if (!shm_addrs[entry]) {
    void *addr = mmap(NULL, pagesize, PROT_READ | PROT_WRITE, MAP_SHARED,
                      shm_fd, entry * pagesize);
    if (addr == (void *)-1)
...
/* We'd like lookup to be fast. To that end, we use a static list indexed by
 * handle. This is copied and adapted from the fd cache code. */
...
static struct esync *add_to_list(HANDLE handle, enum esync_type type, int fd,
                                 void *shm) {
  ...
  {
    if (!entry)
      esync_list[0] = esync_list_initial_block;
    else {
...
NTSTATUS esync_release_semaphore(HANDLE handle, unsigned int count,
                                 ULONG *prev) {
  struct esync *obj;
  ...
  } while (InterlockedCompareExchange(&semaphore->count, count + current,
                                      current) != current);
...
NTSTATUS esync_create_event(HANDLE *handle, ACCESS_MASK access,
                            const OBJECT_ATTRIBUTES *attr,
                            EVENT_TYPE event_type, BOOLEAN initial) {
  enum esync_type type =
      (event_type == SynchronizationEvent ? ESYNC_AUTO_EVENT
                                          : ESYNC_MANUAL_EVENT);
...
NTSTATUS esync_open_event(HANDLE *handle, ACCESS_MASK access,
  ...
  return open_esync(ESYNC_AUTO_EVENT, handle, access,
                    attr); /* doesn't matter which */
}
...
NTSTATUS esync_reset_event(HANDLE handle) {
  ...
  if (InterlockedExchange(&event->signaled, 0) ||
      obj->type == ESYNC_AUTO_EVENT) {
    if (read(obj->fd, &value, sizeof(value)) == -1 && errno != EWOULDBLOCK &&
        errno != EAGAIN) {
      ERR("read: %s\n", strerror(errno));
    }
  }
...
NTSTATUS esync_query_event(HANDLE handle, void *info, ULONG *ret_len) {
  ...
  out->EventType = (obj->type == ESYNC_AUTO_EVENT ? SynchronizationEvent
                                                  : NotificationEvent);
  if (ret_len)
    *ret_len = sizeof(*out);
      ...
      if (obj) {
        ...
        case ESYNC_MUTEX: {
          ...
          if (mutex->tid == GetCurrentThreadId()) {
            ...
            if ((size = read(obj->fd, &value, sizeof(value))) ==
                sizeof(value)) {
              if (mutex->tid == ~0) {
    ...
    while (1) {
      ...
      if (ret > 0) {
        ...
        for (i = 0; i < count; i++) {
          ...
          if (obj) {
            if (obj->type == ESYNC_MANUAL_EVENT ||
                obj->type == ESYNC_MANUAL_SERVER || obj->type == ESYNC_QUEUE) {
              ...
              if ((size = read(fds[i].fd, &value, sizeof(value))) ==
                  sizeof(value)) {
                /* We found our object. */
                TRACE("Woken up by handle %p [%d].\n", handles[i], i);
...
NTSTATUS esync_wait_objects(DWORD count, const HANDLE *handles,
                            BOOLEAN wait_any, BOOLEAN alertable,
                            const LARGE_INTEGER *timeout) {
  BOOL msgwait = FALSE;
  struct esync *obj;
...
void esync_init(void) {
  ...
  if ((shm_fd = shm_open(shm_name, O_RDWR, 0644)) == -1) {
     ...
     */
    if (errno == ENOENT)
      ERR("Failed to open esync shared memory file; make sure no stale "
