...
static void fixup_so_resources(IMAGE_RESOURCE_DIRECTORY *dir, BYTE *root,
                               int delta) {
  IMAGE_RESOURCE_DIRECTORY_ENTRY *entry =
      (IMAGE_RESOURCE_DIRECTORY_ENTRY *)(dir + 1);
  unsigned int i;

  for (i = 0; i < dir->NumberOfNamedEntries + dir->NumberOfIdEntries;
       i++, entry++) {
    void *ptr = root + entry->OffsetToDirectory;
    if (entry->DataIsDirectory)
      fixup_so_resources(ptr, root, delta);
    else
      fixup_rva_dwords(&((IMAGE_RESOURCE_DATA_ENTRY *)ptr)->OffsetToData, delta,
                       1);
  }
}
...
static void set_max_limit(int limit) {
  ...
  if (!getrlimit(limit, &rlimit)) {
    ...
    if (limit == RLIMIT_NOFILE) {
      ...
      size = sizeof(nlimit);
      if (sysctlbyname("kern.maxfilesperproc", &nlimit, &size, NULL, 0) != 0 ||
          nlimit < OPEN_MAX)
...
static char *build_relative_path(const char *base, const char *from,
                                 const char *dest) {
  const char *start;
  char *ret;
...
static int build_path_and_exec(pid_t *pid, const char *dir, const char *name,
                               char **argv) {
  int ret;
...
static void init_paths(char *argv[]) {
  ...
  if (!dladdr(init_paths, &info) ||
      !(ntdll_dir = realpath_dirname(info.dli_fname)))
    fatal_error("cannot get path to ntdll.so\n");

  if (!(build_dir = remove_tail(ntdll_dir, "/dlls/ntdll"))) {
    if (!(dll_dir = remove_tail(ntdll_dir, so_dir)))
      dll_dir = ntdll_dir;
...
#elif defined(__FreeBSD__) || defined(__DragonFly__)
    {
      ...
      if (path &&
          !sysctl(pathname, ARRAY_SIZE(pathname), path, &path_size, NULL, 0))
...
static void preloader_exec(char **argv) {
  ...
  if (strlen(p) > 2 && !strcmp(p + strlen(p) - 2, "64"))
    preloader = "wine64-preloader";
 ...
 *           exec_wineloader
 *
 * argv[0] and argv[1] must be reserved for the preloader and loader
 * respectively.
 */
NTSTATUS exec_wineloader(char **argv, int socketfd,
  ...
  snprintf(preloader_reserve, sizeof(preloader_reserve),
           "WINEPRELOADRESERVE=%x%08x-%x%08x", (UINT)(res_start >> 32),
           (UINT)res_start, (UINT)(res_end >> 32), (UINT)res_end);
...
static int exec_wineserver(pid_t *pid, char **argv) {
  ...
  if (build_dir) {
    if (!is_win64) /* look for 64-bit server */
    {
      ...
      if (loader &&
          !build_path_and_exec(pid, loader, "../server/wineserver", argv))
        return 0;
    }
  ...
  if (!build_path_and_exec(pid, bin_dir, "wineserver", argv))
    return 0;
  if ((path = getenv("WINESERVER")) &&
      !build_path_and_exec(pid, "", path, argv))
    return 0;
 ...
 */
BOOLEAN KeAddSystemServiceTable(ULONG_PTR *funcs, ULONG_PTR *counters,
                                ULONG limit, BYTE *arguments, ULONG index) {
...
static NTSTATUS map_so_dll(const IMAGE_NT_HEADERS *nt_descr, HMODULE module) {
  ...
  unsigned int i;
  DWORD size =
      (sizeof(IMAGE_DOS_HEADER) + sizeof(builtin_signature) +
       sizeof(IMAGE_NT_HEADERS) + nb_sections * sizeof(IMAGE_SECTION_HEADER));
  ...
  *nt = *nt_descr;
  ...
  code_end = data_start;
  data_end =
      (nt->OptionalHeader.SizeOfImage + delta + align_mask) & ~align_mask;
#endif
  ...
  sec->Characteristics = (IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_WRITE |
                          IMAGE_SCN_MEM_READ);
  sec++;

  for (i = 0; i < nt->OptionalHeader.NumberOfRvaAndSizes; i++)
    fixup_rva_dwords(&nt->OptionalHeader.DataDirectory[i].VirtualAddress, delta,
                     1);
  ...
  if (dir->Size) {
    IMAGE_IMPORT_DESCRIPTOR *imports =
        (IMAGE_IMPORT_DESCRIPTOR *)(addr + dir->VirtualAddress);
    ...
    while (imports->Name) {
      ...
      if (imports->OriginalFirstThunk)
        fixup_rva_names((UINT_PTR *)(addr + imports->OriginalFirstThunk),
                        delta);
  ...
  if (dir->Size) {
    IMAGE_EXPORT_DIRECTORY *exports =
        (IMAGE_EXPORT_DIRECTORY *)(addr + dir->VirtualAddress);
    ...
    fixup_rva_dwords((DWORD *)(addr + exports->AddressOfNames), delta,
                     exports->NumberOfNames);
    fixup_rva_ptrs(addr + exports->AddressOfFunctions, addr,
  ...
  if (dir->Size) {
    IMAGE_DELAYLOAD_DESCRIPTOR *imports =
        (IMAGE_DELAYLOAD_DESCRIPTOR *)(addr + dir->VirtualAddress);
...
static ULONG_PTR find_ordinal_export(HMODULE module,
                                     const IMAGE_EXPORT_DIRECTORY *exports,
                                     DWORD ordinal) {
  const DWORD *functions =
      (const DWORD *)((BYTE *)module + exports->AddressOfFunctions);
...
static ULONG_PTR find_named_export(HMODULE module,
                                   const IMAGE_EXPORT_DIRECTORY *exports,
                                   const char *name) {
  const WORD *ordinals =
      (const WORD *)((BYTE *)module + exports->AddressOfNameOrdinals);
  const DWORD *names =
      (const DWORD *)((BYTE *)module + exports->AddressOfNames);
...
static const void *get_module_data_dir(HMODULE module, ULONG dir, ULONG *size) {
  const IMAGE_NT_HEADERS *nt =
      get_rva(module, ((IMAGE_DOS_HEADER *)module)->e_lfanew);
...
static void fill_builtin_image_info(void *module, pe_image_info_t *info) {
  ...
  const IMAGE_NT_HEADERS *nt =
      (IMAGE_NT_HEADERS *)((const BYTE *)dos + dos->e_lfanew);
...
static NTSTATUS dlopen_dll(const char *so_name, UNICODE_STRING *nt_name,
  ...
  fill_builtin_image_info(module, image_info);
  if (prefer_native &&
      (image_info->dll_charact & IMAGE_DLLCHARACTERISTICS_PREFER_NATIVE)) {
...
static NTSTATUS wow64_load_so_dll(void *args) {
  return STATUS_INVALID_IMAGE_FORMAT;
}
static NTSTATUS wow64_unwind_builtin_dll(void *args) {
...
static inline char *prepend_build_dir_path(char *ptr, const char *ext,
                                           const char *arch_dir,
                                           const char *top_dir) {
  char *name = ptr;
 ...
 */
static NTSTATUS open_dll_file(const char *name, OBJECT_ATTRIBUTES *attr,
                              HANDLE *mapping) {
  LARGE_INTEGER size;
  NTSTATUS status;
  HANDLE handle;

  if ((status = open_unix_file(
           &handle, name, GENERIC_READ | SYNCHRONIZE, attr, 0,
...
static NTSTATUS open_builtin_pe_file(const char *name, OBJECT_ATTRIBUTES *attr,
  ...
  *module = NULL;
  ...
  if (!status) {
    status =
        virtual_map_builtin_module(mapping, module, size, image_info, limit_low,
                                   limit_high, machine, prefer_native);
    NtClose(mapping);
  }
  return status;
}
 ...
 *           find_builtin_dll
 */
static NTSTATUS find_builtin_dll(UNICODE_STRING *nt_name, void **module,
                                 SIZE_T *size_ptr,
                                 SECTION_IMAGE_INFORMATION *image_info,
                                 ULONG_PTR limit_low, ULONG_PTR limit_high,
                                 USHORT search_machine, USHORT load_machine,
                                 BOOL prefer_native) {
  ...
  BOOL try_so = (search_machine == current_machine &&
                 (!load_machine || load_machine == search_machine));
  ...
  if (build_dir)
    maxlen = strlen(build_dir) + sizeof("/programs/") + len;
  maxlen = max(maxlen, dll_path_maxlen + 1) + len + sizeof("/aarch64-windows") +
           sizeof(".so");
  ...
  if (build_dir) {
    ...
    if (try_so) {
      ...
      status =
          open_builtin_so_file(ptr, &attr, module, image_info, prefer_native);
      if (status != STATUS_DLL_NOT_FOUND)
        goto done;
    }
 ...
 * Return STATUS_IMAGE_ALREADY_LOADED if we should keep the native one that we
 * have found.
 */
NTSTATUS load_builtin(const pe_image_info_t *image_info, WCHAR *filename,
                      USHORT machine, SECTION_IMAGE_INFORMATION *info,
                      void **module, SIZE_T *size, ULONG_PTR limit_low,
  ...
  if (image_info->wine_builtin) {
    if (loadorder == LO_NATIVE)
      return STATUS_IMAGE_ALREADY_LOADED;
    loadorder = LO_BUILTIN_NATIVE; /* load builtin, then fallback to the file we
                                      found */
  } else if (image_info->wine_fakedll) {
    TRACE("%s is a fake Wine dll\n", debugstr_w(filename));
    if (loadorder == LO_NATIVE)
      return STATUS_DLL_NOT_FOUND;
    loadorder = LO_BUILTIN; /* builtin with no fallback since mapping a fake dll
                               is not useful */
  }

  if (is_arm64ec() && image_info->is_hybrid &&
  ...
  default:
    status =
        find_builtin_dll(&nt_name, module, size, info, limit_low, limit_high,
...
static const WCHAR *get_machine_wow64_dir(WORD machine) {
  static const WCHAR system32[] = {
      '\\', '?',  '?', '\\', 'C', ':', '\\', 'w', 'i', 'n', 'd',  'o', 'w',
      's',  '\\', 's', 'y',  's', 't', 'e',  'm', '3', '2', '\\', 0};
  static const WCHAR syswow64[] = {
      '\\', '?',  '?', '\\', 'C', ':', '\\', 'w', 'i', 'n', 'd',  'o', 'w',
      's',  '\\', 's', 'y',  's', 'w', 'o',  'w', '6', '4', '\\', 0};
  static const WCHAR sysarm32[] = {
      '\\', '?',  '?', '\\', 'C', ':', '\\', 'w', 'i', 'n', 'd',  'o', 'w',
      's',  '\\', 's', 'y',  's', 'a', 'r',  'm', '3', '2', '\\', 0};
 ...
 *           open_main_image
 */
static NTSTATUS open_main_image(WCHAR *image, void **module,
 ...
 *           load_main_exe
 */
NTSTATUS load_main_exe(const WCHAR *dos_name, const char *unix_name,
                       const WCHAR *curdir, USHORT load_machine, WCHAR **image,
                       void **module) {
  ...
  if (unix_name && unix_name[0] == '/' && !stat(unix_name, &st)) {
    if ((status = unix_to_nt_file_name(unix_name, image)))
      goto failed;
    init_unicode_string(&nt_name, *image);
    loadorder = get_load_order(&nt_name);
  ...
  if (!dos_name) {
    ...
    ntdll_umbstowcs(unix_name, strlen(unix_name) + 1, tmp,
                    strlen(unix_name) + 1);
  }
  contains_path = (wcschr(dos_name, '/') || wcschr(dos_name, '\\') ||
                   (dos_name[0] && dos_name[1] == ':'));

  if ((status = get_full_path(dos_name, curdir, image)))
    goto failed;
  free(tmp);

  init_unicode_string(&nt_name, *image);
  if (loadorder == LO_INVALID)
    loadorder = get_load_order(&nt_name);
  ...
  /* if path is in system dir, we can load the builtin even if the file itself
   * doesn't exist */
...
NTSTATUS load_start_exe(WCHAR **image, void **module) {
  static const WCHAR startW[] = {'s', 't', 'a', 'r', 't',
                                 '.', 'e', 'x', 'e', 0};
  UNICODE_STRING nt_name;
  unsigned int status;
  SIZE_T size;
...
#define GET_FUNC(name)                                                         \
  ...
  *p__wine_unixlib_handle = (UINT_PTR)unix_call_funcs;
  if (p__wine_unix_call_dispatcher_arm64ec) {
    /* redirect __wine_unix_call_dispatcher to
...
#undef GET_FUNC
  ...
  p__wine_ctrl_routine =
      (void *)find_named_export(module, exports, "__wine_ctrl_routine");

#ifdef _WIN64
  {
    unixlib_handle_t *p__wine_unixlib_handle =
        (void *)find_named_export(module, exports, "__wine_unixlib_handle");
 ...
 */
ULONG_PTR redirect_arm64ec_rva(void *base, ULONG_PTR rva,
                               const IMAGE_ARM64EC_METADATA *metadata) {
  const IMAGE_ARM64EC_REDIRECTION_ENTRY *map =
      get_rva(base, metadata->RedirectionMetadata);
...
static void redirect_ntdll_functions(HMODULE module) {
  ...
  if (!(loadcfg = get_module_data_dir(module, IMAGE_DIRECTORY_ENTRY_LOAD_CONFIG,
                                      NULL)))
    return;
...
#define REDIRECT(name)                                                         \
  p##name =                                                                    \
      get_rva(module, redirect_arm64ec_rva(                                    \
                          module, (char *)p##name - (char *)module, metadata))
...
static void load_apiset_dll(void) {
  static WCHAR path[] = {
      '\\', '?', '?', '\\', 'C', ':', '\\', 'w', 'i', 'n',  'd', 'o', 'w', 's',
      '\\', 's', 'y', 's',  't', 'e', 'm',  '3', '2', '\\', 'a', 'p', 'i', 's',
      'e',  't', 's', 'c',  'h', 'e', 'm',  'a', '.', 'd',  'l', 'l', 0};
  const char *pe_dir = get_pe_dir(current_machine);
  const IMAGE_NT_HEADERS *nt;
  ...
  if (build_dir)
    asprintf(&name, "%s/dlls/apisetschema%s/apisetschema.dll", build_dir,
             pe_dir);
  else
    asprintf(&name, "%s%s/apisetschema.dll", dll_dir, pe_dir);
  status = open_unix_file(
      &handle, name, GENERIC_READ | SYNCHRONIZE, &attr, 0,
      FILE_SHARE_READ | FILE_SHARE_DELETE, FILE_OPEN,
      FILE_SYNCHRONOUS_IO_NONALERT | FILE_NON_DIRECTORY_FILE, NULL, 0);
  free(name);
  ...
  if (!status) {
    status = NtCreateSection(
        &mapping, STANDARD_RIGHTS_REQUIRED | SECTION_QUERY | SECTION_MAP_READ,
  ...
  if (!status) {
    ...
    for (i = 0; i < nt->FileHeader.NumberOfSections; i++, sec++) {
      ...
      if (sec->PointerToRawData < size &&
          ...
          map->Version == 6 && map->Size <= sec->Misc.VirtualSize) {
        peb->ApiSetMap = map;
        if (wow_peb)
...
static void load_wow64_ntdll(USHORT machine) {
  static const WCHAR ntdllW[] = {'n', 't', 'd', 'l', 'l',
                                 '.', 'd', 'l', 'l', 0};
  SECTION_IMAGE_INFORMATION info;
  UNICODE_STRING nt_name;
  void *module;
  ...
  status = find_builtin_dll(&nt_name, &module, &size, &info, 0, 0, machine, 0,
                            FALSE);
  if (status == STATUS_IMAGE_NOT_AT_BASE)
    status = virtual_relocate_module(module);
  if (status)
...
static ULONG_PTR get_image_address(void) {
  ...
  if (task_info(mach_task_self(), TASK_DYLD_INFO, (task_info_t)&dyld_info,
                &size) == KERN_SUCCESS)
