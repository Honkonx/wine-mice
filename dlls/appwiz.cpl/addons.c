...
static const addon_info_t addons_info[] = {
    {GECKO_VERSION, L"wine-gecko-" GECKO_VERSION "-" GECKO_ARCH ".msi",
     L"gecko", GECKO_SHA, "http://source.winehq.org/winegecko.php", L"MSHTML",
     L"GeckoUrl", L"GeckoCabDir", MAKEINTRESOURCEW(ID_DWL_GECKO_DIALOG)},
    {MONO_VERSION, L"wine-mono-" MONO_VERSION "-" MONO_ARCH ".msi", L"mono",
     MONO_SHA, "http://source.winehq.org/winemono.php", L"Dotnet", L"MonoUrl",
     L"MonoCabDir", MAKEINTRESOURCEW(ID_DWL_MONO_DIALOG)}};
...
static BOOL sha_check(const WCHAR *file_name) {
  ...
  if (!BCryptHash(BCRYPT_SHA256_ALG_HANDLE, NULL, 0, (UCHAR *)file_map, size,
                  sha, sizeof(sha))) {
...
static enum install_res install_from_file(const WCHAR *dir, const WCHAR *subdir,
                                          const WCHAR *file_name) {
  ...
  file = CreateFileW(canonical_path, GENERIC_READ,
  ...
  if (!path)
    return INSTALL_FAILED;
  if (len > 6)
    ret = install_file((path[5] == ':') ? path + 4 : path); /* remove '\\?\' */
  free(path);
  return ret;
}
...
static enum install_res install_from_default_dir(void) {
  ...
  if ((package_dir = _wgetenv(L"WINEBUILDDIR"))) {
    ...
    package_dir = _wgetenv(L"WINEDATADIR");
  ...
  if (ret == INSTALL_NEXT)
    ret = install_from_file(L"\\\\?\\unix" INSTALL_DATADIR "/wine/",
                            addon->subdir_name, addon->file_name);
  if (ret == INSTALL_NEXT && strcmp("" INSTALL_DATADIR, "/usr/share") != 0)
    ret = install_from_file(L"\\\\?\\unix/usr/share/wine/", addon->subdir_name,
                            addon->file_name);
  if (ret == INSTALL_NEXT)
    ret = install_from_file(L"\\\\?\\unix/opt/wine/", addon->subdir_name,
                            addon->file_name);
  return ret;
}
...
static WCHAR *get_cache_file_name(BOOL ensure_exists) {
  ...
  if (xdg_dir && *xdg_dir) {
    if (!(cache_dir = HeapAlloc(GetProcessHeap(), 0,
                                wcslen(xdg_dir) * sizeof(WCHAR) +
                                    sizeof(L"\\\\?\\unix"))))
      return NULL;
  ...
  if (ensure_exists && !CreateDirectoryW(cache_dir, NULL) &&
  ...
  size = lstrlenW(cache_dir) + ARRAY_SIZE(L"\\wine") +
         lstrlenW(addon->file_name) + 1;
  ...
  if (ensure_exists && !CreateDirectoryW(ret, NULL) &&
      GetLastError() != ERROR_ALREADY_EXISTS) {
    WARN("%s does not exist and could not be created (%lu)\n", debugstr_w(ret),
         GetLastError());
    free(ret);
...
static HRESULT WINAPI InstallCallback_OnDataAvailable(
...
static HRESULT WINAPI InstallCallback_OnObjectAvailable(
...
static const IBindStatusCallbackVtbl InstallCallbackVtbl = {
    InstallCallback_QueryInterface,   InstallCallback_AddRef,
    InstallCallback_Release,          InstallCallback_OnStartBinding,
    InstallCallback_GetPriority,      InstallCallback_OnLowResource,
    InstallCallback_OnProgress,       InstallCallback_OnStopBinding,
    InstallCallback_GetBindInfo,      InstallCallback_OnDataAvailable,
    InstallCallback_OnObjectAvailable};

static IBindStatusCallback InstallCallback = {&InstallCallbackVtbl};

static HRESULT WINAPI InstallCallbackBindInfo_QueryInterface(
    IInternetBindInfo *iface, REFIID riid, void **ppv) {
...
static HRESULT WINAPI InstallCallbackBindInfo_GetBindInfo(
    IInternetBindInfo *iface, DWORD *bindf, BINDINFO *bindinfo) {
  ERR("\n");
  return E_NOTIMPL;
}

static HRESULT WINAPI InstallCallbackBindInfo_GetBindString(
    IInternetBindInfo *iface, ULONG string_type, WCHAR **strs, ULONG cnt,
    ULONG *fetched) {
  switch (string_type) {
...
static const IInternetBindInfoVtbl InstallCallbackBindInfoVtbl = {
    InstallCallbackBindInfo_QueryInterface, InstallCallbackBindInfo_AddRef,
    InstallCallbackBindInfo_Release, InstallCallbackBindInfo_GetBindInfo,
...
static void append_url_params(WCHAR *url) {
  ...
  len += MultiByteToWideChar(CP_ACP, 0, GECKO_ARCH, sizeof(GECKO_ARCH),
                             url + len, size / sizeof(WCHAR) - len) -
         1;
  lstrcpyW(url + len, L"&v=");
  len += lstrlenW(L"&v=");
  len += MultiByteToWideChar(CP_ACP, 0, addon->version, -1, url + len,
                             size / sizeof(WCHAR) - len) -
         1;
  lstrcpyW(url + len, L"&winev=");
  len += lstrlenW(L"&winev=");
  MultiByteToWideChar(CP_ACP, 0, p_wine_get_version ? p_wine_get_version() : 0,
...
static LPWSTR get_url(void) {
  ...
  hkey = open_config_key();
  if (hkey) {
    res = RegQueryValueExW(hkey, addon->url_config_key, NULL, &type,
                           (LPBYTE)url, &returned_size);
    RegCloseKey(hkey);
    if (res == ERROR_SUCCESS && type == REG_SZ)
      goto found;
  }

  MultiByteToWideChar(CP_ACP, 0, addon->url_default, -1, url,
                      size / sizeof(WCHAR));
...
static BOOL start_download(void) {
  ...
  if (SUCCEEDED(hres)) {
    hres = IMoniker_BindToStorage(mon, bind_ctx, NULL, &IID_IUnknown,
                                  (void **)&tmp);
...
static void run_winebrowser(const WCHAR *url) {
  ...
  si.cb = sizeof(si);
  ret = CreateProcessW(app, args, NULL, NULL, FALSE, DETACHED_PROCESS, NULL,
                       NULL, &si, &pi);
  free(args);
  if (ret) {
...
BOOL install_addon(addon_t addon_type) {
  ...
  p_wine_get_version = (void *)GetProcAddress(GetModuleHandleW(L"ntdll.dll"),
                                              "wine_get_version");

  /*
   * Try to find addon .msi file in following order:
   * - directory stored in $dir_config_key value of
   * HKCU/Software/Wine/$config_key key
   * - $datadir/$addon_subdir/
   * - $INSTALL_DATADIR/wine/$addon_subdir/
   * - /usr/share/wine/$addon_subdir/
   * - /opt/wine/$addon_subdir/
   * - download from URL stored in $url_config_key value of
   * HKCU/Software/Wine/$config_key key
   */
  if (install_from_registered_dir() == INSTALL_NEXT &&
      install_from_default_dir() == INSTALL_NEXT &&
      install_from_cache() == INSTALL_NEXT && (url = get_url()))
