 ...
 *      + C++ support
 *      + we should store the underlying type for an enum in the symt_enum
 * struct
 *      + for enums, we store the names & values (associated to the enum type),
 *        but those values are not directly usable from a debugger (that's why,
 * I assume, that we have also to define constants for enum values, as Codeview
 * does BTW.
 *      + SymEnumTypes should only return *user* defined types (UDT,
 * typedefs...) not all the types stored/used in the modules (like char*)
 *  - SymGetLine{Next|Prev} don't work as expected (they don't seem to work
 * across functions, and even across function blocks...). Basically, for *Next*
 * to work it requires an address after the prolog of the func (the base address
 * of the func doesn't work)
 *  - most options (dbghelp_options) are not used (loading lines...)
 *  - in symbol lookup by name, we don't use RE everywhere we should. Moreover,
 * when we're supposed to use RE, it doesn't make use of our hash tables.
 * Therefore, we could use hash if name isn't a RE, and fall back to a full
 * search when we get a full RE
 *  - msc:
 ...
 *      + C++ management
 *  - stabs:
 *      + when, in a same module, the same definition is used in several
 * compilation units, we get several definitions of the same object (especially
 *        struct/union). we should find a way not to duplicate them
 *      + in some cases (dlls/user/dialog16.c DIALOG_GetControl16), the same
 * static global variable is defined several times (at different scopes). We are
 *        getting several of those while looking for a unique symbol. Part of
 * the issue is that we don't give a scope to a static variable inside a
 * function
 *      + C++ management
 */
...
const char *wine_dbgstr_addr(const ADDRESS64 *addr) {
  ...
  case AddrMode1616:
    return wine_dbg_sprintf("1616<%04x:%04lx>", addr->Segment,
                            (DWORD)addr->Offset);
  case AddrMode1632:
    return wine_dbg_sprintf("1632<%04x:%08lx>", addr->Segment,
                            (DWORD)addr->Offset);
  case AddrModeReal:
    return wine_dbg_sprintf("real<%04x:%04lx>", addr->Segment,
                            (DWORD)addr->Offset);
  default:
    return "unknown";
  }
...
BOOL WINAPI SymGetSearchPath(HANDLE hProcess, PSTR szSearchPath,
                             DWORD SearchPathLength) {
  WCHAR *buffer =
      HeapAlloc(GetProcessHeap(), 0, SearchPathLength * sizeof(WCHAR));
  BOOL ret = FALSE;

  if (buffer) {
    ret = SymGetSearchPathW(hProcess, buffer, SearchPathLength);
    if (ret)
      WideCharToMultiByte(CP_ACP, 0, buffer, SearchPathLength, szSearchPath,
 ...
 */
static BOOL WINAPI process_invade_cb(PCWSTR name, ULONG64 base, ULONG size,
                                     PVOID user) {
...
const struct cpu *process_get_cpu(const struct process *pcs) {
  ...
  /* return cpu of main module, which is the first module in process's modules
   * list */
 ...
 *
 */
static BOOL check_live_target(struct process *pcs, BOOL wow64,
                              BOOL child_wow64) {
  PROCESS_BASIC_INFORMATION pbi;
  DWORD64 base = 0, env = 0;
  const char *peb_addr;

  if (!GetProcessId(pcs->handle))
    return FALSE;
  if (GetEnvironmentVariableA("DBGHELP_NOLIVE", NULL, 0))
    return FALSE;
  ...
  if (!pcs->is_64bit) {
    ...
    if (!wow64 && child_wow64)
      /* current process is 64bit, while child process is 32 bit, need to read
       * 32bit PEB */
      peb_addr += 0x1000;
    if (!ReadProcessMemory(pcs->handle, peb_addr, &peb32, sizeof(peb32), NULL))
      return FALSE;
  ...
  if (pcs->is_64bit || base == 0) {
    ...
    if (!pcs->is_64bit)
      peb_addr -= 0x1000; /* PEB32 => PEB64 */
    if (!ReadProcessMemory(pcs->handle, peb_addr, &peb, sizeof(peb), NULL))
      return FALSE;
    ...
    if (pcs->is_64bit)
      ReadProcessMemory(
          pcs->handle,
          (char *)(ULONG_PTR)peb.ProcessParameters +
              FIELD_OFFSET(RTL_USER_PROCESS_PARAMETERS, Environment),
          &env, sizeof(env), NULL);
  }
  ...
  if (env) {
    ...
    do {
      ...
      if (!(new_buf = realloc(buf, buf_size + read_size)))
        break;
      buf = new_buf;

      if (!read_process_memory(pcs, env, (char *)buf + buf_size, read_size))
        break;
      for (i = buf_size / sizeof(WCHAR);
           i < (buf_size + read_size) / sizeof(WCHAR); i++) {
        if (buf[i])
          continue;
        if (last_null + 1 == i) {
 ...
 *   we won't be able to make the peering between a builtin PE module and its
 * ELF counterpart. Hence we won't be able to provide the requested debug
 ...
 */
BOOL WINAPI SymInitializeW(HANDLE hProcess, PCWSTR UserSearchPath,
                           BOOL fInvadeProcess) {
  struct process *pcs;
  ...
  if (hProcess == INVALID_HANDLE_VALUE) {
    ...
    /* MSDN says to only call this function once unless SymCleanup() has been
       called since the last call. It also says to call SymRefreshModuleList()
       instead if you just want the module list refreshed. Native still returns
       TRUE even if the process has already been initialized. */
    return TRUE;
  }
  ...
  pcs->loader =
      &no_loader_ops; /* platform-specific initialization will override it if
                         loader debug info can be found */
...
BOOL WINAPI SymCleanup(HANDLE hProcess) {
  ...
  for (ppcs = &process_first; *ppcs; ppcs = &(*ppcs)->next) {
    if ((*ppcs)->handle == hProcess) {
      while ((*ppcs)->lmodules)
        module_remove(*ppcs, (*ppcs)->lmodules);
 ...
 *
 */
DWORD WINAPI SymGetOptions(void) { return dbghelp_options; }
...
BOOL WINAPI SymSetContext(HANDLE hProcess, PIMAGEHLP_STACK_FRAME StackFrame,
  ...
  if (!(pcs = process_find_by_handle(hProcess)))
    return FALSE;
  if (pcs->ctx_frame.ReturnOffset == StackFrame->ReturnOffset &&
    ...
    TRACE("Setting same frame {rtn=%I64x frm=%I64x stk=%I64x}\n",
          pcs->ctx_frame.ReturnOffset, pcs->ctx_frame.FrameOffset,
...
BOOL WINAPI SymSetScopeFromAddr(HANDLE hProcess, ULONG64 addr) {
  ...
  if ((sym = symt_find_symbol_at(pair.effective, addr)) != NULL &&
...
BOOL WINAPI SymSetScopeFromIndex(HANDLE hProcess, ULONG64 addr, DWORD index) {
  ...
  if (!symt_check_tag(sym, SymTagFunction))
    return FALSE;

  pair.pcs->localscope_pc =
      ((struct symt_function *)sym)
          ->ranges[0]
          .low; /* FIXME of FuncDebugStart when it exists? */
 ...
 */
BOOL WINAPI SymSetScopeFromInlineContext(HANDLE hProcess, ULONG64 addr,
                                         DWORD inlinectx) {
  struct module_pair pair;
  ...
  case IFC_MODE_REGULAR:
    return SymSetScopeFromAddr(hProcess, addr);
  default:
    SetLastError(ERROR_INVALID_PARAMETER);
    return FALSE;
  }
 ...
 */
static BOOL CALLBACK reg_cb64to32(HANDLE hProcess, ULONG action, ULONG64 data,
                                  ULONG64 user) {
  struct process *pcs = process_find_by_handle(hProcess);
  void *data32;
...
BOOL pcs_callback(const struct process *pcs, ULONG action, void *data) {
  ...
  if (!pcs->reg_is_unicode) {
    ...
    case CBA_DEFERRED_SYMBOL_LOAD_START:
      ...
      WideCharToMultiByte(CP_ACP, 0, idslW->FileName, -1, idsl.FileName,
                          sizeof(idsl.FileName), NULL, NULL);
  ...
  return pcs->reg_cb(pcs->handle, action, (ULONG64)(DWORD_PTR)data,
                     pcs->reg_user);
}
 ...
 */
static BOOL sym_register_cb(HANDLE hProcess, PSYMBOL_REGISTERED_CALLBACK64 cb,
                            PSYMBOL_REGISTERED_CALLBACK cb32, DWORD64 user,
                            BOOL unicode) {
  struct process *pcs = process_find_by_handle(hProcess);
...
BOOL WINAPI SymRegisterCallback(HANDLE hProcess,
  ...
  return sym_register_cb(hProcess, reg_cb64to32, CallbackFunction,
                         (DWORD_PTR)UserContext, FALSE);
}
 ...
 */
LPAPI_VERSION WINAPI ImagehlpApiVersion(VOID) { return &api_version; }
