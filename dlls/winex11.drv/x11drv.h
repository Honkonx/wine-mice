...
extern BOOL X11DRV_Arc(PHYSDEV dev, INT left, INT top, INT right, INT bottom,
                       INT xstart, INT ystart, INT xend, INT yend);
extern BOOL X11DRV_Chord(PHYSDEV dev, INT left, INT top, INT right, INT bottom,
                         INT xstart, INT ystart, INT xend, INT yend);
extern BOOL X11DRV_Ellipse(PHYSDEV dev, INT left, INT top, INT right,
                           INT bottom);
extern BOOL X11DRV_ExtFloodFill(PHYSDEV dev, INT x, INT y, COLORREF color,
                                UINT fillType);
extern BOOL X11DRV_FillPath(PHYSDEV dev);
extern BOOL X11DRV_GetDeviceGammaRamp(PHYSDEV dev, LPVOID ramp);
extern BOOL X11DRV_GetICMProfile(PHYSDEV dev, BOOL allow_default, LPDWORD size,
                                 LPWSTR filename);
...
extern UINT X11DRV_GetSystemPaletteEntries(PHYSDEV dev, UINT start, UINT count,
...
extern BOOL X11DRV_Pie(PHYSDEV dev, INT left, INT top, INT right, INT bottom,
                       INT xstart, INT ystart, INT xend, INT yend);
extern BOOL X11DRV_PolyPolygon(PHYSDEV dev, const POINT *pt, const INT *counts,
                               UINT polygons);
extern BOOL X11DRV_PolyPolyline(PHYSDEV dev, const POINT *pt,
...
extern BOOL X11DRV_Rectangle(PHYSDEV dev, INT left, INT top, INT right,
                             INT bottom);
extern BOOL X11DRV_RoundRect(PHYSDEV dev, INT left, INT top, INT right,
                             INT bottom, INT ell_width, INT ell_height);
extern HBRUSH X11DRV_SelectBrush(PHYSDEV dev, HBRUSH hbrush,
                                 const struct brush_pattern *pattern);
extern HPEN X11DRV_SelectPen(PHYSDEV dev, HPEN hpen,
                             const struct brush_pattern *pattern);
...
extern INT X11DRV_ToUnicodeEx(UINT virtKey, UINT scanCode,
                              const BYTE *lpKeyState, LPWSTR bufW,
...
extern LONG X11DRV_ChangeDisplaySettings(LPDEVMODEW displays,
                                         LPCWSTR primary_name, HWND hwnd,
                                         DWORD flags, LPVOID lpvoid);
extern UINT
X11DRV_UpdateDisplayDevices(const struct gdi_device_manager *device_manager,
                            void *param);
extern BOOL X11DRV_CreateDesktop(const WCHAR *name, UINT width, UINT height);
extern BOOL X11DRV_CreateWindow(HWND hwnd);
extern LRESULT X11DRV_DesktopWindowProc(HWND hwnd, UINT msg, WPARAM wp,
                                        LPARAM lp);
...
extern void X11DRV_SetLayeredWindowAttributes(HWND hwnd, COLORREF key,
...
extern LRESULT X11DRV_ClipboardWindowProc(HWND hwnd, UINT msg, WPARAM wp,
                                          LPARAM lp);
...
extern BOOL X11DRV_WindowPosChanging(HWND hwnd, UINT swp_flags, BOOL shaped,
                                     const struct window_rects *rects);
extern BOOL X11DRV_GetWindowStyleMasks(HWND hwnd, UINT style, UINT ex_style,
                                       UINT *style_mask, UINT *ex_style_mask);
extern BOOL X11DRV_CreateWindowSurface(HWND hwnd, BOOL layered,
                                       const RECT *surface_rect,
                                       struct window_surface **surface);
extern void X11DRV_MoveWindowBits(HWND hwnd,
                                  const struct window_rects *old_rects,
                                  const struct window_rects *new_rects,
                                  const RECT *valid_rects);
extern void X11DRV_WindowPosChanged(HWND hwnd, HWND insert_after,
                                    UINT swp_flags, BOOL fullscreen,
                                    const struct window_rects *new_rects,
                                    struct window_surface *surface);
extern BOOL X11DRV_SystemParametersInfo(UINT action, UINT int_param,
                                        void *ptr_param, UINT flags);
...
extern DWORD copy_image_bits(BITMAPINFO *info, BOOL is_r8g8b8, XImage *image,
                             ...
                             struct bitblt_coords *coords, const int *mapping,
                             unsigned int zeropad_mask);
extern Pixmap create_pixmap_from_image(HDC hdc, const XVisualInfo *vis,
                                       const BITMAPINFO *info,
                                       const struct gdi_image_bits *bits,
                                       UINT coloruse);
extern DWORD get_pixmap_image(Pixmap pixmap, int width, int height,
                              const XVisualInfo *vis, BITMAPINFO *info,
                              struct gdi_image_bits *bits);
...
extern void execute_rop(X11DRV_PDEVICE *physdev, Pixmap src_pixmap, GC gc,
                        const RECT *visrect, DWORD rop);

extern BOOL X11DRV_SetupGCForPatBlt(X11DRV_PDEVICE *physDev, GC gc,
                                    BOOL fMapColors);
...
extern UINT X11DRV_VulkanInit(UINT, void *,
                              const struct vulkan_driver_funcs **);

extern struct format_entry *import_xdnd_selection(Display *display, Window win,
                                                  Atom selection, Atom *targets,
...
#define X11DRV_PALETTE_FIXED                                                   \
  0x0001 /* read-only colormap - have to use XAllocColor (if not virtual) */
#define X11DRV_PALETTE_VIRTUAL                                                 \
  0x0002 /* no mapping needed - pixel == pixel color */
...
extern COLORREF X11DRV_PALETTE_GetColor(X11DRV_PDEVICE *physDev,
                                        COLORREF color);
...
static inline const char *debugstr_color(COLORREF color) {
  ...
  return wine_dbg_sprintf("RGB(%02x,%02x,%02x)", GetRValue(color),
...
extern void set_dc_drawable(HDC hdc, Drawable drawable, const RECT *rect,
                            int mode);
...
static inline struct x11drv_thread_data *x11drv_thread_data(void) {
  return (struct x11drv_thread_data *)(UINT_PTR)NtUserGetThreadInfo()
      ->driver_data;
}
...
extern void X11DRV_register_event_handler(int type,
...
extern Bool (*pXGetEventData)(Display *display,
                              XEvent /*XGenericEventCookie*/ *event);
extern void (*pXFreeEventData)(Display *display,
                               XEvent /*XGenericEventCookie*/ *event);
...
struct x11drv_win_data {
  ...
  struct window_rects
      rects; /* window rects in monitor DPI, relative to parent client area */
  ...
  UINT add_taskbar
      : 1; /* does window should be added to taskbar regardless of style */
  UINT net_wm_fullscreen_monitors_set
      : 1;                /* is _NET_WM_FULLSCREEN_MONITORS set */
...
extern Window create_client_window(HWND hwnd, const XVisualInfo *visual,
                                   Colormap colormap);
extern void detach_client_window(struct x11drv_win_data *data,
                                 Window client_window);
extern void attach_client_window(struct x11drv_win_data *data,
                                 Window client_window);
extern void destroy_client_window(HWND hwnd, Window client_window);
extern void set_window_visual(struct x11drv_win_data *data,
                              const XVisualInfo *vis, BOOL use_alpha);
...
extern DROPFILES *file_list_to_drop_files(const void *data, size_t size,
                                          size_t *ret_size);
extern DROPFILES *uri_list_to_drop_files(const void *data, size_t size,
                                         size_t *ret_size);
...
extern BOOL process_events(Display *display,
                           Bool (*filter)(Display *, XEvent *, XPointer),
                           ULONG_PTR arg);
extern BOOL X11DRV_ProcessEvents(DWORD mask);
extern HWND *build_hwnd_list(void);

typedef int (*x11drv_error_callback)(Display *display, XErrorEvent *event,
                                     void *arg);

extern void X11DRV_expect_error(Display *display,
                                x11drv_error_callback callback, void *arg);
...
struct x11drv_settings_handler {
  ...
  /* get_id() will be called to map a device name, e.g., \\.\DISPLAY1 to a
   * driver specific id. Following functions use this id to identify the device.
   *
   * Return FALSE if the device cannot be found and TRUE on success */
  BOOL (*get_id)(const WCHAR *device_name, BOOL is_primary,
                 x11drv_settings_id *id);

  /* get_modes() will be called to get a list of supported modes of the device
   * of id in modes with respect to flags, which could be 0, EDS_RAWMODE or
   * EDS_ROTATEDMODE. If the implementation uses dmDriverExtra then every
   * DEVMODEW in the list must have the same dmDriverExtra value
   *
   * Following fields in DEVMODE must be valid:
   * dmSize, dmDriverExtra, dmFields, dmDisplayOrientation, dmBitsPerPel,
   * dmPelsWidth, dmPelsHeight, dmDisplayFlags and dmDisplayFrequency
   *
   * Return FALSE on failure with parameters unchanged and error code set.
   * Return TRUE on success */
  BOOL (*get_modes)(x11drv_settings_id id, DWORD flags, DEVMODEW **modes,
                    UINT *mode_count, BOOL full);

  /* free_modes() will be called to free the mode list returned from get_modes()
   */
  void (*free_modes)(DEVMODEW *modes);

  /* get_current_mode() will be called to get the current display mode of the
   * device of id
   *
   * Following fields in DEVMODE must be valid:
   * dmFields, dmDisplayOrientation, dmBitsPerPel, dmPelsWidth, dmPelsHeight,
  ...
  /* set_current_mode() will be called to change the display mode of the display
   * device of id. mode must be a valid mode from get_modes() with optional
   * fields, such as dmPosition set.
   *
...
#define NEXT_DEVMODEW(mode)                                                    \
  ((DEVMODEW *)((char *)((mode) + 1) + (mode)->dmDriverExtra))

extern void
X11DRV_Settings_SetHandler(const struct x11drv_settings_handler *handler);

extern void X11DRV_init_desktop(Window win, unsigned int width,
                                unsigned int height);
...
struct x11drv_display_device_handler {
  ...
  /* get_gpus will be called to get a list of GPUs. First GPU has to be where
  ...
  /* get_adapters will be called to get a list of adapters in EnumDisplayDevices
   * context under a GPU. The first adapter has to be primary if GPU is primary.
   *
   * Return FALSE on failure with parameters unchanged */
  BOOL (*get_adapters)(ULONG_PTR gpu_id, struct x11drv_adapter **adapters,
                       int *count);

  /* get_monitors will be called to get a list of monitors in EnumDisplayDevices
   * context under an adapter. The first monitor has to be primary if adapter is
   * primary.
   *
   * Return FALSE on failure with parameters unchanged */
  BOOL (*get_monitors)(ULONG_PTR adapter_id, struct gdi_monitor **monitors,
                       int *count);
   ...
   * This function pointer is optional and can be NULL when driver doesn't
   * support it */
  void (*register_event_handlers)(void);
};

extern void X11DRV_DisplayDevices_SetHandler(
    const struct x11drv_display_device_handler *handler);
...
static inline BOOL is_window_rect_mapped(const RECT *rect) {
  ...
  return (rect->left < virtual_rect.right && rect->top < virtual_rect.bottom &&
...
static inline UINT get_palette_entries(HPALETTE palette, UINT start, UINT count,
                                       PALETTEENTRY *entries) {
  return NtGdiDoPalette(palette, start, count, entries, NtGdiGetPaletteEntries,
                        TRUE);
}

/* user helpers */

static inline LRESULT send_message(HWND hwnd, UINT msg, WPARAM wparam,
                                   LPARAM lparam) {
  return NtUserMessageCall(hwnd, msg, wparam, lparam, NULL, NtUserSendMessage,
                           FALSE);
}
...
static inline LRESULT send_message_timeout(HWND hwnd, UINT msg, WPARAM wparam,
  ...
  struct send_message_timeout_params params = {.flags = flags,
                                               .timeout = timeout};
  LRESULT res = NtUserMessageCall(hwnd, msg, wparam, lparam, &params,
                                  NtUserSendMessageTimeout, FALSE);
  if (res_ptr)
    *res_ptr = params.result;
  return res;
}

static inline BOOL send_notify_message(HWND hwnd, UINT msg, WPARAM wparam,
                                       LPARAM lparam) {
  return NtUserMessageCall(hwnd, msg, wparam, lparam, 0,
                           NtUserSendNotifyMessage, FALSE);
}

/* per-monitor DPI aware NtUserChildWindowFromPointEx call */
static inline HWND child_window_from_point(HWND parent, LONG x, LONG y,
                                           UINT flags) {
  UINT context =
      NtUserSetThreadDpiAwarenessContext(NTUSER_DPI_PER_MONITOR_AWARE_V2);
...
static inline BOOL intersect_rect(RECT *dst, const RECT *src1,
                                  const RECT *src2) {
