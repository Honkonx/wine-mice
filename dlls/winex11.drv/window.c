...
static const WCHAR foreign_window_prop[] = {
    '_', '_', 'w', 'i', 'n', 'e', '_', 'x', '1', '1', '_', 'f', 'o',
    'r', 'e', 'i', 'g', 'n', '_', 'w', 'i', 'n', 'd', 'o', 'w', 0};
static const WCHAR whole_window_prop[] = {
    '_', '_', 'w', 'i', 'n', 'e', '_', 'x', '1', '1', '_', 'w',
    'h', 'o', 'l', 'e', '_', 'w', 'i', 'n', 'd', 'o', 'w', 0};
static const WCHAR clip_window_prop[] = {'_', '_', 'w', 'i', 'n', 'e', '_', 'x',
                                         '1', '1', '_', 'c', 'l', 'i', 'p', '_',
                                         'w', 'i', 'n', 'd', 'o', 'w', 0};
...
static void remove_startup_notification(Display *display, Window window) {
  ...
  if (!(id = getenv("DESKTOP_STARTUP_ID")) || !id[0])
    return;

  if ((src = strstr(id, "_TIME")))
    update_user_time(atol(src + 5));
  ...
  while (srclen > 0) {
    ...
    XSendEvent(display, DefaultRootWindow(display), False, PropertyChangeMask,
               &xevent);
...
HWND *build_hwnd_list(void) {
  ...
  for (;;) {
    if (!(list = malloc(count * sizeof(*list))))
      return NULL;
...
static BOOL has_owned_popups(HWND hwnd) {
  ...
  if (!(list = build_hwnd_list()))
    return FALSE;

  for (i = 0; list[i] != HWND_BOTTOM; i++) {
    if (list[i] == hwnd)
      break; /* popups are always above owner */
    if (NtUserGetWindowRelative(list[i], GW_OWNER) != hwnd)
      continue;
    if ((ret = is_managed(list[i])))
      break;
  }
 ...
 */
static BOOL is_window_managed(HWND hwnd, UINT swp_flags,
  ...
  if (style & WS_POPUP) {
    ...
    if (window_rect->left <= mi.rcWork.left &&
        window_rect->right >= mi.rcWork.right &&
        window_rect->top <= mi.rcWork.top &&
        window_rect->bottom >= mi.rcWork.bottom)
      return TRUE;
  }
 ...
 */
static inline BOOL is_window_resizable(struct x11drv_win_data *data,
                                       DWORD style) {
  if (style & WS_THICKFRAME)
    return TRUE;
 ...
 *              get_mwm_decorations
 */
static unsigned long get_mwm_decorations_for_style(DWORD style,
                                                   DWORD ex_style) {
  unsigned long ret = 0;
  ...
  if ((style & WS_CAPTION) == WS_CAPTION) {
    ...
    if (style & WS_MINIMIZEBOX)
      ret |= MWM_DECOR_MINIMIZE;
    if (style & WS_MAXIMIZEBOX)
      ret |= MWM_DECOR_MAXIMIZE;
  }
  if (ex_style & WS_EX_DLGMODALFRAME)
    ret |= MWM_DECOR_BORDER;
...
static int get_window_attributes(struct x11drv_win_data *data,
  ...
  attr->colormap =
      data->whole_colormap ? data->whole_colormap : default_colormap;
  attr->save_under =
      ((NtUserGetClassLongW(data->hwnd, GCL_STYLE) & CS_SAVEBITS) != 0);
  attr->bit_gravity = NorthWestGravity;
  attr->backing_store = NotUseful;
  attr->border_pixel = 0;
  attr->event_mask =
      (ExposureMask | PointerMotionMask | ButtonPressMask | ButtonReleaseMask |
       EnterWindowMask | KeyPressMask | KeyReleaseMask | FocusChangeMask |
...
static void sync_empty_window_shape(struct x11drv_win_data *data,
  ...
  if (IsRectEmpty(&data->rects.window)) /* set an empty shape */
  {
    ...
    XShapeCombineRectangles(data->display, data->whole_window, ShapeBounding, 0,
                            0, &empty_rect, 1, ShapeSet, YXBanded);
  } else {
    XShapeCombineMask(gdi_display, data->whole_window, ShapeBounding, 0, 0,
                      None, ShapeSet);
    /* invalidate surface shape to make sure it gets updated again */
    if (surface)
      window_surface_set_shape(surface, 0);
  }
#endif
}
...
static void sync_window_region(struct x11drv_win_data *data, HRGN win_region) {
  ...
  if (!data->whole_window)
    return;
  if (client_side_graphics)
    return; /* use surface shape instead */
  data->shaped = FALSE;

  if (hrgn ==
      (HRGN)1) /* hack: win_region == 1 means retrieve region from server */
  {
    if (!(hrgn = NtGdiCreateRectRgn(0, 0, 0, 0)))
      return;
  ...
  if (!hrgn) {
    XShapeCombineMask(data->display, data->whole_window, ShapeBounding, 0, 0,
                      None, ShapeSet);
  } else {
    RGNDATA *pRegionData;
    ...
    if ((pRegionData = X11DRV_GetRegionData(hrgn, 0))) {
      XShapeCombineRectangles(data->display, data->whole_window, ShapeBounding,
                              data->rects.window.left -
                                  data->rects.visible.left,
 ...
 *              sync_window_opacity
 */
static void sync_window_opacity(Display *display, Window win, BYTE alpha,
                                DWORD flags) {
...
static void sync_window_text(Display *display, Window win, const WCHAR *text) {
  ...
  if (XmbTextListToTextProperty(display, &buffer, 1, XStdICCTextStyle, &prop) ==
      Success) {
  ...
  */
  XChangeProperty(display, win, x11drv_atom(_NET_WM_NAME),
                  x11drv_atom(UTF8_STRING), 8, PropModeReplace,
                  (unsigned char *)utf8_buffer, count);
 ...
 */
static unsigned long *get_bitmap_argb(HDC hdc, HBITMAP color, HBITMAP mask,
                                      unsigned int *size) {
  ...
  if (!NtGdiExtGetObjectW(color, sizeof(bm), &bm))
    return NULL;
  ...
  *size = bm.bmWidth * bm.bmHeight + 2;
  if (!(bits = malloc(*size * sizeof(*bits))))
    goto failed;
  ptr = (unsigned int *)bits;
  if (!NtGdiGetDIBitsInternal(hdc, color, 0, bm.bmHeight, ptr + 2, info,
                              DIB_RGB_COLORS, 0, 0))
    goto failed;
  ...
  for (i = 0; i < bm.bmWidth * bm.bmHeight; i++)
    if ((has_alpha = (ptr[i + 2] & 0xff000000) != 0))
      break;
  ...
  if (!has_alpha) {
    ...
    if (!(mask_bits = malloc(info->bmiHeader.biSizeImage)))
      goto failed;
    if (!NtGdiGetDIBitsInternal(hdc, mask, 0, bm.bmHeight, mask_bits, info,
                                DIB_RGB_COLORS, 0, 0))
      goto failed;
 ...
 *              create_icon_pixmaps
 */
static BOOL create_icon_pixmaps(HDC hdc, const ICONINFO *icon, Pixmap *icon_ret,
                                Pixmap *mask_ret) {
  char buffer[FIELD_OFFSET(BITMAPINFO, bmiColors[256])];
  BITMAPINFO *info = (BITMAPINFO *)buffer;
  ...
  if (!(lines = NtGdiGetDIBitsInternal(hdc, icon->hbmColor, 0, 0, NULL, info,
                                       DIB_RGB_COLORS, 0, 0)))
    goto failed;
  if (!(bits.ptr = malloc(info->bmiHeader.biSizeImage)))
    goto failed;
  if (!NtGdiGetDIBitsInternal(hdc, icon->hbmColor, 0, lines, bits.ptr, info,
                              DIB_RGB_COLORS, 0, 0))
    goto failed;

  color_pixmap =
      create_pixmap_from_image(hdc, &vis, info, &bits, DIB_RGB_COLORS);
  free(bits.ptr);
  bits.ptr = NULL;
  if (!color_pixmap)
    goto failed;
  ...
  info->bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
  info->bmiHeader.biBitCount = 0;
  if (!(lines = NtGdiGetDIBitsInternal(hdc, icon->hbmMask, 0, 0, NULL, info,
                                       DIB_RGB_COLORS, 0, 0)))
    goto failed;
  if (!(bits.ptr = malloc(info->bmiHeader.biSizeImage)))
    goto failed;
  if (!NtGdiGetDIBitsInternal(hdc, icon->hbmMask, 0, lines, bits.ptr, info,
                              DIB_RGB_COLORS, 0, 0))
    goto failed;
...
failed:
  if (color_pixmap)
    XFreePixmap(gdi_display, color_pixmap);
  free(bits.ptr);
  return FALSE;
}
...
static void fetch_icon_data(HWND hwnd, HICON icon_big, HICON icon_small) {
  ...
  if (!icon_big) {
    icon_big =
        get_icon_info((HICON)send_message(hwnd, WM_GETICON, ICON_BIG, 0), &ii);
    if (!icon_big)
      icon_big =
          get_icon_info((HICON)NtUserGetClassLongPtrW(hwnd, GCLP_HICON), &ii);
    if (!icon_big) {
  ...
  if (!icon_small) {
    icon_small = get_icon_info(
        (HICON)send_message(hwnd, WM_GETICON, ICON_SMALL, 0), &ii_small);
    if (!icon_small)
      icon_small = get_icon_info(
          (HICON)NtUserGetClassLongPtrW(hwnd, GCLP_HICONSM), &ii_small);
  }
  ...
  if (bits && icon_small) {
    ...
    if ((bits_small = get_bitmap_argb(hDC, ii_small.hbmColor, ii_small.hbmMask,
                                      &size_small)) &&
        (bits_small[0] != bits[0] ||
         bits_small[1] != bits[1])) /* size must be different */
    {
  ...
  if (!create_icon_pixmaps(hDC, &ii, &icon_pixmap, &mask_pixmap))
    icon_pixmap = mask_pixmap = 0;
  ...
  if ((data = get_win_data(hwnd))) {
    if (data->icon_pixmap)
      XFreePixmap(gdi_display, data->icon_pixmap);
    if (data->icon_mask)
      XFreePixmap(gdi_display, data->icon_mask);
    free(data->icon_bits);
    ...
    if (icon_pixmap)
      XFreePixmap(gdi_display, icon_pixmap);
    if (mask_pixmap)
...
static void set_size_hints(struct x11drv_win_data *data, DWORD style) {
  ...
  if (!(style & (WS_MINIMIZE | WS_MAXIMIZE))) {
    ...
    {
      POINT pt = virtual_screen_to_root(data->rects.visible.left,
 ...
 *              set_mwm_hints
 */
static void set_mwm_hints(struct x11drv_win_data *data, UINT style,
                          UINT ex_style) {
  MwmHints mwm_hints;
  ...
  if (data->hwnd == NtUserGetDesktopWindow()) {
    ...
    if (is_desktop_fullscreen()) {
      ...
      mwm_hints.functions |=
          MWM_FUNC_RESIZE; /* some WMs need this to make it fullscreen */
    } else
      mwm_hints.decorations = MWM_DECOR_TITLE | MWM_DECOR_BORDER |
    ...
    if (!(style & WS_DISABLED)) {
      ...
      if (style & WS_MINIMIZE)
        mwm_hints.functions |= MWM_FUNC_MINIMIZE | MWM_FUNC_MAXIMIZE;
    }
  }

  TRACE("%p setting mwm hints to %lx,%lx (style %x exstyle %x)\n", data->hwnd,
        mwm_hints.decorations, mwm_hints.functions, style, ex_style);

  mwm_hints.flags = MWM_HINTS_FUNCTIONS | MWM_HINTS_DECORATIONS;
  mwm_hints.input_mode = 0;
  mwm_hints.status = 0;
  XChangeProperty(data->display, data->whole_window,
                  x11drv_atom(_MOTIF_WM_HINTS), x11drv_atom(_MOTIF_WM_HINTS),
 ...
 *              set_style_hints
 */
static void set_style_hints(struct x11drv_win_data *data, DWORD style,
                            DWORD ex_style) {
  ...
  XChangeProperty(data->display, data->whole_window,
                  x11drv_atom(_NET_WM_WINDOW_TYPE), XA_ATOM, 32,
  ...
  if (data->icon_bits)
    XChangeProperty(data->display, data->whole_window,
                    x11drv_atom(_NET_WM_ICON), XA_CARDINAL, 32, PropModeReplace,
...
static void set_initial_wm_hints(Display *display, Window window) {
  ...
  if (use_take_focus)
    protocols[i++] = x11drv_atom(WM_TAKE_FOCUS);
  XChangeProperty(display, window, x11drv_atom(WM_PROTOCOLS), XA_ATOM, 32,
                  PropModeReplace, (unsigned char *)protocols, i);
  ...
  /* set the pid. together, these properties are needed so the window manager
   * can kill us if we freeze */
  i = getpid();
  XChangeProperty(display, window, x11drv_atom(_NET_WM_PID), XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)&i, 1);

  XChangeProperty(display, window, x11drv_atom(XdndAware), XA_ATOM, 32,
                  PropModeReplace, (unsigned char *)&dndVersion, 1);
  ...
  XChangeProperty(display, window, x11drv_atom(_NET_WM_HWND), XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)&hwnd, 2);
}
...
static void make_owner_managed(HWND hwnd) {
  static const UINT flags = SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE |
                            SWP_NOMOVE | SWP_NOREDRAW | SWP_DEFERERASE |
                            SWP_NOSENDCHANGING | SWP_STATECHANGED;
  HWND owner;

  if (!(owner = NtUserGetWindowRelative(hwnd, GW_OWNER)))
    return;
  if (is_managed(owner))
    return;
  if (!is_managed(hwnd))
...
Window init_clip_window(void) {
  ...
  if (!data->clip_window && (data->clip_window = (Window)NtUserGetProp(
                                 NtUserGetDesktopWindow(), clip_window_prop))) {
...
void update_user_time(Time time) {
  if (!user_time_window) {
    Window win =
        XCreateWindow(gdi_display, root_window, -1, -1, 1, 1, 0, CopyFromParent,
                      InputOnly, CopyFromParent, 0, NULL);
    if (InterlockedCompareExchangePointer((void **)&user_time_window,
                                          (void *)win, 0))
  ...
  if (!last_user_time || (long)(time - last_user_time) > 0) {
    last_user_time = time;
    XChangeProperty(gdi_display, user_time_window,
                    x11drv_atom(_NET_WM_USER_TIME), XA_CARDINAL, 32,
...
static void update_desktop_fullscreen(Display *display) {
  ...
  xev.xclient.data.l[0] =
      is_desktop_fullscreen() ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE;
...
/* Update _NET_WM_FULLSCREEN_MONITORS when _NET_WM_STATE_FULLSCREEN is set to
 * support fullscreen windows spanning multiple monitors */
static void update_net_wm_fullscreen_monitors(struct x11drv_win_data *data) {
  ...
  /* If the current display device handler cannot detect dynamic device changes,
   * do not use _NET_WM_FULLSCREEN_MONITORS because
   * xinerama_get_fullscreen_monitors() may report wrong indices because of
   * stale xinerama monitor information */
  ...
  /* If _NET_WM_FULLSCREEN_MONITORS is not set and the fullscreen monitors are
   * spanning only one monitor then do not set _NET_WM_FULLSCREEN_MONITORS.
   *
   * If _NET_WM_FULLSCREEN_MONITORS is set then the property needs to be updated
   * because it can't be deleted by sending a _NET_WM_FULLSCREEN_MONITORS client
   * message to the root window according to the wm-spec version 1.5. Having the
   * window spanning more than two monitors also needs the property set. In
   * other cases, _NET_WM_FULLSCREEN_MONITORS doesn't need to be set. What's
   * more, setting _NET_WM_FULLSCREEN_MONITORS adds a constraint on Mutter so
   * that such a window can't be moved to another monitor by using the
   * Shift+Super+Up/Down/Left/Right shortcut. So the property should be added
   * only when necessary. */
  if (monitors[0] == monitors[1] && monitors[1] == monitors[2] &&
      monitors[2] == monitors[3] && !data->net_wm_fullscreen_monitors_set)
    return;

  if (!data->mapped) {
    XChangeProperty(data->display, data->whole_window,
                    x11drv_atom(_NET_WM_FULLSCREEN_MONITORS), XA_CARDINAL, 32,
...
void update_net_wm_states(struct x11drv_win_data *data) {
  ...
  if (!data->add_taskbar) {
    ...
    else if (!(ex_style & WS_EX_APPWINDOW) &&
  ...
  if (!data->mapped) /* set the _NET_WM_STATE atom directly */
  {
    ...
    XChangeProperty(data->display, data->whole_window,
                    x11drv_atom(_NET_WM_STATE), XA_ATOM, 32, PropModeReplace,
  ...
  } else /* ask the window manager to do it for us */
  {
    ...
    for (i = 0; i < NB_NET_WM_STATES; i++) {
      TRACE("setting wm state %u for window %p/%lx to %u prev %u\n", i,
            data->hwnd, data->whole_window, (new_state & (1 << i)) != 0,
            (data->net_wm_state & (1 << i)) != 0);

      xev.xclient.data.l[0] =
          (new_state & (1 << i)) ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE;
      xev.xclient.data.l[1] = X11DRV_Atoms[net_wm_state_atoms[i] - FIRST_XATOM];
      xev.xclient.data.l[2] =
          ((net_wm_state_atoms[i] == XATOM__NET_WM_STATE_MAXIMIZED_VERT)
...
void read_net_wm_states(Display *display, struct x11drv_win_data *data) {
  ...
  if (!XGetWindowProperty(display, data->whole_window,
                          x11drv_atom(_NET_WM_STATE), 0, 65536 / sizeof(CARD32),
 ...
 *     set_xembed_flags
 */
static void set_xembed_flags(struct x11drv_win_data *data,
                             unsigned long flags) {
...
static void sync_window_position(struct x11drv_win_data *data, UINT swp_flags) {
  ...
  if (!(style & WS_MAXIMIZE) || !data->managed) {
    ...
    if (changes.width <= 0 || changes.height <= 0)
      changes.width = changes.height = 1;
    if (changes.width > 65535)
      changes.width = 65535;
    if (changes.height > 65535)
      changes.height = 65535;
    mask |= CWWidth | CWHeight;
  }

  /* only the size is allowed to change for the desktop window or systray docked
   * windows */
  ...
  XReconfigureWMWindow(data->display, data->whole_window, data->vis.screen,
                       mask, &changes);

  TRACE("win %p/%lx pos %d,%d,%dx%d after %lx changes=%x serial=%lu\n",
        data->hwnd, data->whole_window, (int)data->rects.visible.left,
        (int)data->rects.visible.top,
...
static void sync_client_position(struct x11drv_win_data *data,
  ...
  if (changes.x != old_rects->client.left - old_rects->visible.left)
    mask |= CWX;
  if (changes.y != old_rects->client.top - old_rects->visible.top)
    mask |= CWY;

  if (mask) {
    TRACE("setting client win %lx pos %d,%d changes=%x\n", data->client_window,
          changes.x, changes.y, mask);
 ...
 */
static void move_window_bits(HWND hwnd, Window window,
                             const struct window_rects *old_rects,
  ...
  rgn = NtGdiCreateRectRgn(dst_rect.left, dst_rect.top, dst_rect.right,
                           dst_rect.bottom);
  ...
  NtGdiExtEscape(hdc_dst, NULL, 0, X11DRV_ESCAPE, sizeof(code), (LPSTR)&code, 0,
                 NULL);
  ...
  NtGdiExtEscape(hdc_dst, NULL, 0, X11DRV_ESCAPE, sizeof(code), (LPSTR)&code,
                 sizeof(rgn), (LPSTR)&rgn);

  NtUserReleaseDC(hwnd, hdc_dst);
  if (hdc_src != hdc_dst)
    NtUserReleaseDC(parent, hdc_src);
 ...
 *              get_dummy_parent
 *
 * Create a dummy parent window for child windows that don't have a true X11
 * parent.
 */
Window get_dummy_parent(void) {
  ...
  if (!dummy_parent) {
    ...
    {
      static XRectangle empty_rect;
      dummy_parent = XCreateWindow(
          gdi_display, root_window, 0, 0, 1, 1, 0, default_visual.depth,
          InputOutput, default_visual.visual,
          CWColormap | CWBorderPixel | CWOverrideRedirect, &attrib);
      XShapeCombineRectangles(gdi_display, dummy_parent, ShapeBounding, 0, 0,
                              &empty_rect, 1, ShapeSet, YXBanded);
    }
#else
    dummy_parent =
        XCreateWindow(gdi_display, root_window, -1, -1, 1, 1, 0,
                      default_visual.depth, InputOutput, default_visual.visual,
                      CWColormap | CWBorderPixel | CWOverrideRedirect, &attrib);
    WARN("Xshape support is not compiled in. Applications under XWayland may "
...
static void client_window_events_enable(struct x11drv_win_data *data,
                                        Window client_window) {
  XSaveContext(data->display, client_window, winContext, (char *)data->hwnd);
  XSync(data->display,
        False); /* make sure client_window is known from data->display */
  XSelectInput(data->display, client_window, ExposureMask);
}

static void client_window_events_disable(struct x11drv_win_data *data,
                                         Window client_window) {
  XSelectInput(data->display, client_window, 0);
  XFlush(data->display); /* make sure XSelectInput doesn't use client_window
                            after this point */
...
void detach_client_window(struct x11drv_win_data *data, Window client_window) {
  if (data->client_window != client_window || !client_window)
    return;
...
void attach_client_window(struct x11drv_win_data *data, Window client_window) {
  ...
  if (data->whole_window) {
    ...
    XReparentWindow(gdi_display, client_window, data->whole_window,
                    data->rects.client.left - data->rects.visible.left,
...
Window create_client_window(HWND hwnd, const XVisualInfo *visual,
  ...
  if (!data) {
    /* explicitly create data for HWND_MESSAGE windows since they can be used
     * for OpenGL */
    HWND parent = NtUserGetAncestor(hwnd, GA_PARENT);
    if (parent == NtUserGetDesktopWindow() ||
        NtUserGetAncestor(parent, GA_PARENT))
      return 0;
    if (!(data = alloc_win_data(thread_init_display(), hwnd)))
      return 0;
    NtUserGetClientRect(hwnd, &data->rects.client,
  ...
  XSync(gdi_display,
        False); /* make sure whole_window is known from gdi_display */
  ret = data->client_window = XCreateWindow(
      gdi_display, data->whole_window ? data->whole_window : dummy_parent, x, y,
      cx, cy, 0, default_visual.depth, InputOutput, visual->visual,
      CWBitGravity | CWWinGravity | CWBackingStore | CWColormap | CWBorderPixel,
      &attr);
  if (data->client_window) {
    XMapWindow(gdi_display, data->client_window);
    if (data->whole_window) {
      XFlush(gdi_display); /* make sure client_window is created for
                              XSelectInput */
...
static void create_whole_window(struct x11drv_win_data *data) {
  ...
  if (!data->managed &&
      is_window_managed(data->hwnd, SWP_NOACTIVATE, &data->rects.window)) {
  ...
  if (data->vis.visualid != default_visual.visualid)
    data->whole_colormap = XCreateColormap(data->display, root_window,
                                           data->vis.visual, AllocNone);

  mask = get_window_attributes(data, &attr);

  if (!(cx = data->rects.visible.right - data->rects.visible.left))
    cx = 1;
  else if (cx > 65535)
    cx = 65535;
  if (!(cy = data->rects.visible.bottom - data->rects.visible.top))
    cy = 1;
  else if (cy > 65535)
    cy = 65535;

  pos =
      virtual_screen_to_root(data->rects.visible.left, data->rects.visible.top);
  ...
  XSaveContext(data->display, data->whole_window, winContext,
               (char *)data->hwnd);
  ...
  if (IsRectEmpty(&data->rects.window))
    sync_empty_window_shape(data, NULL);
  else if (win_rgn)
    sync_window_region(data, win_rgn);

  /* set the window opacity */
  if (!NtUserGetLayeredWindowAttributes(data->hwnd, &key, &alpha,
                                        &layered_flags))
    layered_flags = 0;
  sync_window_opacity(data->display, data->whole_window, alpha, layered_flags);

  XFlush(data->display); /* make sure the window exists before we start painting
                            to it */
 ...
 */
static void destroy_whole_window(struct x11drv_win_data *data,
  ...
  if (!data->whole_window) {
    ...
    if (!already_destroyed) {
      XSync(gdi_display, False); /* make sure XReparentWindow requests have
                                    completed before destroying whole_window */
      XDestroyWindow(data->display, data->whole_window);
    }
  }
  if (data->whole_colormap)
    XFreeColormap(data->display, data->whole_colormap);
...
void set_window_visual(struct x11drv_win_data *data, const XVisualInfo *vis,
  ...
  if (!data->use_alpha == !use_alpha && same_visual)
    return;
...
void X11DRV_SetWindowText(HWND hwnd, LPCWSTR text) {
  ...
  if ((win = X11DRV_get_whole_window(hwnd)) &&
      win != DefaultRootWindow(gdi_display)) {
...
void X11DRV_SetWindowStyle(HWND hwnd, INT offset, STYLESTRUCT *style) {
  ...
  if (offset == GWL_EXSTYLE &&
      (changed & WS_EX_LAYERED)) /* changing WS_EX_LAYERED resets attributes */
  {
    data->layered = FALSE;
...
void X11DRV_DestroyWindow(HWND hwnd) {
  ...
  destroy_whole_window(data, FALSE);
  if (thread_data->last_focus == hwnd)
    thread_data->last_focus = 0;
  if (thread_data->last_xic_hwnd == hwnd)
    thread_data->last_xic_hwnd = 0;
  if (data->icon_pixmap)
    XFreePixmap(gdi_display, data->icon_pixmap);
  if (data->icon_mask)
    XFreePixmap(gdi_display, data->icon_mask);
...
static BOOL create_desktop_win_data(Window win, HWND hwnd) {
  ...
  release_win_data(data);
  if (thread_data->clip_window)
    XReparentWindow(display, thread_data->clip_window, win, 0, 0);
  return TRUE;
}
...
void X11DRV_SetDesktopWindow(HWND hwnd) {
  ...
  if (!width && !height) /* not initialized yet */
  {
    ...
    if (!create_desktop_win_data(root_window, hwnd)) {
      ...
      XChangeProperty(display, root_window, x11drv_atom(_NET_WM_STATE), XA_ATOM,
                      32, PropModeReplace,
...
BOOL X11DRV_CreateWindow(HWND hwnd) {
  if (hwnd == NtUserGetDesktopWindow()) {
    ...
    data->clip_window = XCreateWindow(data->display, root_window, 0, 0, 1, 1, 0,
                                      0, InputOnly, default_visual.visual,
 ...
 */
static struct x11drv_win_data *
X11DRV_create_win_data(HWND hwnd, const struct window_rects *rects) {
  Display *display;
  struct x11drv_win_data *data;
  HWND parent;
  ...
  if (parent == NtUserGetDesktopWindow()) {
    ...
    TRACE("win %p/%lx window %s whole %s client %s\n", hwnd, data->whole_window,
          wine_dbgstr_rect(&data->rects.window),
          wine_dbgstr_rect(&data->rects.visible),
          wine_dbgstr_rect(&data->rects.client));
  }
  return data;
}
...
HWND create_foreign_window(Display *display, Window xwin) {
  ...
  if (!class_registered) {
    ...
    if (!NtUserRegisterClassExWOW(&class, &class_name, &version, NULL, 0, 0,
                                  NULL) &&
  ...
  if (XFindContext(display, xwin, winContext, (char **)&hwnd))
    hwnd = 0;
  if (hwnd)
    return hwnd; /* already created */
  ...
  hwnd = NtUserCreateWindowEx(0, &class_name, &class_name, NULL, style, pos.x,
                              pos.y, attr.width, attr.height, parent, 0, NULL,
                              NULL, 0, NULL, 0, FALSE);
  if (!(data = get_win_data(hwnd))) {
  ...
  TRACE("win %lx parent %p style %08x %s -> hwnd %p\n", xwin, parent, style,
        wine_dbgstr_rect(&data->rects.window), hwnd);
...
void X11DRV_SystrayDockClear(HWND hwnd) {
  ...
  if (win)
    XClearArea(gdi_display, win, 0, 0, 0, 0, True);
}
...
static void get_systray_visual_info(Display *display, Window systray_window,
                                    XVisualInfo *info) {
  ...
  *info = default_visual;
  if (XGetWindowProperty(
          display, systray_window, x11drv_atom(_NET_SYSTEM_TRAY_VISUAL), 0,
...
void X11DRV_GetDC(HDC hdc, HWND hwnd, HWND top, const RECT *win_rect,
  ...
  if (top == hwnd) {
     ...
     */
    if (data && data->whole_window == root_window)
      escape.mode = ClipByChildren;
    release_win_data(data);
  } else {
    /* find the first ancestor that has a drawable */
    for (parent = hwnd; parent && parent != top;
         parent = NtUserGetAncestor(parent, GA_PARENT))
      if ((escape.drawable = X11DRV_get_whole_window(parent)))
        break;
  ...
  if (!escape.drawable)
    return; /* don't create a GC for foreign windows */
...
void X11DRV_ReleaseDC(HWND hwnd, HDC hdc) {
  ...
  OffsetRect(&escape.dc_rect, -2 * escape.dc_rect.left,
             -2 * escape.dc_rect.top);
  NtGdiExtEscape(hdc, NULL, 0, X11DRV_ESCAPE, sizeof(escape), (LPSTR)&escape, 0,
                 NULL);
}
...
BOOL X11DRV_ScrollDC(HDC hdc, INT dx, INT dy, HRGN update) {
  ...
  if (update) {
    ...
    NtGdiExtEscape(hdc, NULL, 0, X11DRV_ESCAPE, sizeof(code), (LPSTR)&code, 0,
                   NULL);

    ret = NtGdiBitBlt(hdc, rect.left, rect.top, rect.right - rect.left,
                      rect.bottom - rect.top, hdc, rect.left - dx,
                      rect.top - dy, SRCCOPY, 0, 0);
...
void X11DRV_SetCapture(HWND hwnd, UINT flags) {
  ...
  if (!(flags & (GUI_INMOVESIZE | GUI_INMENUMODE)))
    return;

  if (hwnd) {
    if (!(data = get_win_data(NtUserGetAncestor(hwnd, GA_ROOT))))
      return;
    if (data->whole_window) {
      XFlush(gdi_display);
  ...
  {
    if (!(data = get_win_data(thread_data->grab_hwnd)))
      return;
    XFlush(gdi_display);
 ...
 */
BOOL X11DRV_WindowPosChanging(HWND hwnd, UINT swp_flags, BOOL shaped,
                              const struct window_rects *rects) {
  struct x11drv_win_data *data = get_win_data(hwnd);
  BOOL ret = FALSE;

  TRACE("hwnd %p, swp_flags %#x, shaped %u, rects %s\n", hwnd, swp_flags,
        shaped, debugstr_window_rects(rects));

  if (!data && !(data = X11DRV_create_win_data(hwnd, rects)))
    return FALSE; /* use default surface */
  data->shaped = shaped;

  /* check if we need to switch the window to managed */
  if (!data->managed && data->whole_window &&
      is_window_managed(hwnd, swp_flags, &rects->window)) {
    TRACE("making win %p/%lx managed\n", hwnd, data->whole_window);
    release_win_data(data);
    unmap_window(hwnd);
    if (!(data = get_win_data(hwnd)))
  ...
  ret =
      !!data->whole_window; /* use default surface if we don't have a window */
  release_win_data(data);
...
void X11DRV_MoveWindowBits(HWND hwnd, const struct window_rects *old_rects,
  ...
  if (EqualRect(&valid_rects[0], &new_rects->window) &&
      EqualRect(&valid_rects[1], &old_rects->window)) {
    /* if we have an X window the bits will be moved by the X server */
    if (!window && (valid_rects[0].left - valid_rects[1].left ||
                    valid_rects[0].top - valid_rects[1].top))
 ...
 */
BOOL X11DRV_GetWindowStyleMasks(HWND hwnd, UINT style, UINT ex_style,
...
void X11DRV_WindowPosChanged(HWND hwnd, HWND insert_after, UINT swp_flags,
  ...
  TRACE("win %p/%lx new_rects %s style %08x flags %08x\n", hwnd,
        data->whole_window, debugstr_window_rects(new_rects), new_style,
        swp_flags);
  ...
  if (data->rects.client.right - data->rects.client.left !=
  ...
  if (data->mapped && event_type != ReparentNotify) {
    if (((swp_flags & SWP_HIDEWINDOW) && !(new_style & WS_VISIBLE)) ||
        (!event_type && !(new_style & WS_MINIMIZE) &&
         !is_window_rect_mapped(&new_rects->window) &&
         is_window_rect_mapped(&old_rects.window))) {
      release_win_data(data);
      unmap_window(hwnd);
      if (was_fullscreen)
  ...
  /* don't change position if we are about to minimize or maximize a managed
   * window */
  if (!event_type && !(data->managed && (swp_flags & SWP_STATECHANGED) &&
  ...
  if ((new_style & WS_VISIBLE) && ((new_style & WS_MINIMIZE) ||
    ...
    if (!data->mapped) {
      ...
      return;
    } else if ((swp_flags & SWP_STATECHANGED) &&
               (!data->iconic != !(new_style & WS_MINIMIZE))) {
      set_wm_hints(data);
      data->iconic = (new_style & WS_MINIMIZE) != 0;
      ...
      if (swp_flags & (SWP_FRAMECHANGED | SWP_STATECHANGED))
        set_wm_hints(data);
      if (!event_type)
        update_net_wm_states(data);
    }
  ...
  XFlush(data->display); /* make sure changes are done before we start painting
                            again */
  release_win_data(data);
}
...
static BOOL hide_icon(struct x11drv_win_data *data) {
  static const WCHAR trayW[] = {'S', 'h', 'e', 'l', 'l', '_', 'T',
                                'r', 'a', 'y', 'W', 'n', 'd', 0};
...
UINT X11DRV_ShowWindow(HWND hwnd, INT cmd, RECT *rect, UINT swp) {
  ...
  if (!data->managed || !data->mapped || data->iconic)
    goto done;
  ...
  TRACE("win %p/%lx cmd %d at %s flags %08x\n", hwnd, data->whole_window, cmd,
        wine_dbgstr_rect(rect), swp);

  XGetGeometry(thread_data->display, data->whole_window, &root, &x, &y, &width,
               &height, &border, &depth);
  XTranslateCoordinates(thread_data->display, data->whole_window, root, 0, 0,
                        &x, &y, &top);
...
void X11DRV_SetWindowIcon(HWND hwnd, UINT type, HICON icon) {
  ...
  if (!data->whole_window)
    goto done;
  release_win_data(
      data); /* release the lock, fetching the icon requires sending messages */

  if (type == ICON_BIG)
    fetch_icon_data(hwnd, icon, 0);
  else
    fetch_icon_data(hwnd, 0, icon);
 ...
 */
void X11DRV_SetLayeredWindowAttributes(HWND hwnd, COLORREF key, BYTE alpha,
                                       DWORD flags) {
  struct x11drv_win_data *data = get_win_data(hwnd);
  ...
  if (data) {
    ...
    if (!data->mapped) /* mapping is delayed until attributes are set */
    {
      ...
      if ((style & WS_VISIBLE) &&
          ((style & WS_MINIMIZE) ||
           is_window_rect_mapped(&data->rects.window))) {
        release_win_data(data);
  ...
  case WM_WINE_DESKTOP_RESIZED:
    if ((data = get_win_data(hwnd))) {
      ...
      if (data->whole_window) {
        ...
        POINT old_pos = {.x = data->rects.visible.left - wp,
                         .y = data->rects.visible.top - lp};
        POINT pos = virtual_screen_to_root(data->rects.visible.left,
                                           data->rects.visible.top);
...
static BOOL is_netwm_supported(Display *display, Atom atom) {
  ...
  if (net_supported_count == -1) {
    ...
    if (!XGetWindowProperty(display, DefaultRootWindow(display),
                            x11drv_atom(_NET_SUPPORTED), 0, ~0UL, False,
...
LRESULT X11DRV_SysCommand(HWND hwnd, WPARAM wparam, LPARAM lparam) {
  ...
  if (!(data = get_win_data(hwnd))) {
    if (wparam == SC_SCREENSAVE && hwnd == NtUserGetDesktopWindow())
      return start_screensaver();
    return -1;
  }
  if (!data->whole_window || !data->managed || !data->mapped)
    goto failed;

  switch (wparam & 0xfff0) {
  case SC_MOVE:
    if (!hittest)
      dir = _NET_WM_MOVERESIZE_MOVE_KEYBOARD;
    else
      dir = _NET_WM_MOVERESIZE_MOVE;
    break;
  case SC_SIZE:
    /* windows without WS_THICKFRAME are not resizable through the window
     * manager */
    if (!(NtUserGetWindowLongW(hwnd, GWL_STYLE) & WS_THICKFRAME))
      goto failed;

    switch (hittest) {
    case WMSZ_LEFT:
      dir = _NET_WM_MOVERESIZE_SIZE_LEFT;
      break;
    case WMSZ_RIGHT:
      dir = _NET_WM_MOVERESIZE_SIZE_RIGHT;
      break;
    case WMSZ_TOP:
      dir = _NET_WM_MOVERESIZE_SIZE_TOP;
      break;
    case WMSZ_TOPLEFT:
      dir = _NET_WM_MOVERESIZE_SIZE_TOPLEFT;
      break;
    case WMSZ_TOPRIGHT:
      dir = _NET_WM_MOVERESIZE_SIZE_TOPRIGHT;
      break;
    case WMSZ_BOTTOM:
      dir = _NET_WM_MOVERESIZE_SIZE_BOTTOM;
      break;
    case WMSZ_BOTTOMLEFT:
      dir = _NET_WM_MOVERESIZE_SIZE_BOTTOMLEFT;
      break;
  ...
  case SC_KEYMENU:
    ...
    if ((WCHAR)lparam)
      goto failed; /* got an explicit char */
    if (NtUserGetWindowLongPtrW(hwnd, GWLP_ID))
      goto failed; /* window has a real menu */
    if (!(NtUserGetWindowLongW(hwnd, GWL_STYLE) & WS_SYSMENU))
      goto failed; /* no system menu */
  ...
  if (NtUserGetWindowLongW(hwnd, GWL_STYLE) & WS_MAXIMIZE)
    goto failed;
...
void X11DRV_FlashWindowEx(FLASHWINFO *pfinfo) {
  ...
  if (data->mapped) {
    ...
    xev.xclient.format = 32;
    xev.xclient.data.l[0] =
        pfinfo->dwFlags ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE;
