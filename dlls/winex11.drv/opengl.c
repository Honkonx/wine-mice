...
struct glx_pixel_format {
  ...
  int fmt_id;
  int render_type;
  DWORD dwFlags; /* We store some PFD_* flags in here for emulated bitmap
                    formats */
};
...
/* Set when GLX_EXT_swap_control_tear is supported, requires
 * GLX_SWAP_CONTROL_EXT */
...
static void dump_PIXELFORMATDESCRIPTOR(const PIXELFORMATDESCRIPTOR *ppfd) {
  TRACE("size %u version %u flags %u type %u color %u %u,%u,%u,%u "
        ...
        ppfd->cColorBits, ppfd->cRedBits, ppfd->cGreenBits, ppfd->cBlueBits,
        ppfd->cAlphaBits, ppfd->cAccumBits, ppfd->cDepthBits,
        ppfd->cStencilBits, ppfd->cAuxBuffers);
#define TEST_AND_DUMP(t, tv)                                                   \
  if ((t) & (tv))                                                              \
  TRACE(#tv " ")
...
#define PUSH1(attribs, att)                                                    \
  do {                                                                         \
    attribs[nAttribs++] = (att);                                               \
  } while (0)
#define PUSH2(attribs, att, value)                                             \
  do {                                                                         \
    attribs[nAttribs++] = (att);                                               \
    attribs[nAttribs++] = (value);                                             \
  } while (0)

/* GLX 1.0 */
static XVisualInfo *(*pglXChooseVisual)(Display *dpy, int screen,
                                        int *attribList);
static GLXContext (*pglXCreateContext)(Display *dpy, XVisualInfo *vis,
                                       GLXContext shareList, Bool direct);
static void (*pglXDestroyContext)(Display *dpy, GLXContext ctx);
static Bool (*pglXMakeCurrent)(Display *dpy, GLXDrawable drawable,
                               GLXContext ctx);
static void (*pglXCopyContext)(Display *dpy, GLXContext src, GLXContext dst,
                               unsigned long mask);
...
static int (*pglXGetFBConfigAttrib)(Display *dpy, GLXFBConfig config,
                                    int attribute, int *value);
static GLXFBConfig *(*pglXGetFBConfigs)(Display *dpy, int screen,
                                        int *nelements);
static XVisualInfo *(*pglXGetVisualFromFBConfig)(Display *dpy,
                                                 GLXFBConfig config);
static GLXPbuffer (*pglXCreatePbuffer)(Display *dpy, GLXFBConfig config,
                                       const int *attribList);
static void (*pglXDestroyPbuffer)(Display *dpy, GLXPbuffer pbuf);
static void (*pglXQueryDrawable)(Display *dpy, GLXDrawable draw, int attribute,
                                 unsigned int *value);
static GLXContext (*pglXCreateNewContext)(Display *dpy, GLXFBConfig config,
                                          int renderType, GLXContext shareList,
                                          Bool direct);
static Bool (*pglXMakeContextCurrent)(Display *dpy, GLXDrawable draw,
                                      GLXDrawable read, GLXContext ctx);
static GLXPixmap (*pglXCreatePixmap)(Display *dpy, GLXFBConfig config,
...
/* GLX Extensions */
static GLXContext (*pglXCreateContextAttribsARB)(Display *dpy,
                                                 GLXFBConfig config,
                                                 GLXContext share_context,
                                                 Bool direct,
                                                 const int *attrib_list);
static void *(*pglXGetProcAddressARB)(const GLubyte *);
static void (*pglXSwapIntervalEXT)(Display *dpy, GLXDrawable drawable,
                                   int interval);
static int (*pglXSwapIntervalSGI)(int);

/* NV GLX Extension */
static void *(*pglXAllocateMemoryNV)(GLsizei size, GLfloat readfreq,
...
static Bool (*pglXQueryCurrentRendererIntegerMESA)(int attribute,
                                                   unsigned int *value);
static const char *(*pglXQueryCurrentRendererStringMESA)(int attribute);
static Bool (*pglXQueryRendererIntegerMESA)(Display *dpy, int screen,
                                            int renderer, int attribute,
                                            unsigned int *value);
static const char *(*pglXQueryRendererStringMESA)(Display *dpy, int screen,
                                                  int renderer, int attribute);

/* OpenML GLX Extensions */
static Bool (*pglXWaitForSbcOML)(Display *dpy, GLXDrawable drawable,
                                 INT64 target_sbc, INT64 *ust, INT64 *msc,
                                 INT64 *sbc);
...
static int GLXErrorHandler(Display *dpy, XErrorEvent *event, void *arg) {
  /* In the future we might want to find the exact X or GLX error to report back
   * to the app */
...
static BOOL X11DRV_WineGL_InitOpenglInfo(void) {
  ...
  if (vis) {
    ...
    if (old_fs != new_fs) {
      ERR("%%fs register corrupted, probably broken ATI driver, disabling "
          "OpenGL.\n");
      ERR("You need to set the \"UseFastTls\" option to \"2\" in your X config "
          "file.\n");
      goto done;
    }
#else
  ...
  if ((win = XCreateWindow(gdi_display, root, -1, -1, 1, 1, 0, vis->depth,
  ...
  if (pglXMakeCurrent(gdi_display, win, ctx) == 0) {
    ERR_(winediag)("Unable to activate OpenGL context, most likely your %s "
                   "OpenGL drivers haven't been "
                   "installed correctly\n",
  ...
  if (!glx_direct) {
    ...
    /* In general indirect rendering on a local X11 server indicates a driver
     * problem. Detect a local X11 server by checking whether the X11 socket is
     * a Unix socket.
     */
    if (!getsockname(fd, (struct sockaddr *)&uaddr, &uaddrlen) &&
      ...
      ERR_(winediag)(
          ...
          "haven't been installed correctly (using GL renderer %s, version "
          "%s).\n",
          is_win64 ? "64-bit" : "32-bit", debugstr_a(gl_renderer),
          debugstr_a(gl_version));
  } else {
    /* In general you would expect that if direct rendering is returned, that
     * you receive hardware accelerated OpenGL rendering. The definition of
     * direct rendering is that rendering is performed client side without
     * sending all GL commands to X using the GLX protocol. When Mesa falls back
     * to software rendering, it shows direct rendering.
     *
     * Depending on the cause of software rendering a different rendering string
     * is shown. In case Mesa fails to load a DRI module 'Software Rasterizer'
     * is returned. When Mesa is compiled as a OpenGL reference driver it shows
     * 'Mesa X11'.
     */
    if (!strcmp(gl_renderer, "Software Rasterizer") ||
        !strcmp(gl_renderer, "Mesa X11"))
      ERR_(winediag)("The Mesa OpenGL driver is using software rendering, most "
                     "likely your %s OpenGL "
...
static void init_opengl(void) {
  ...
  /* No need to load any other libraries as according to the ABI, libGL should
     be self-sufficient and include all dependencies */
  ...
  for (i = 0; i < ARRAY_SIZE(opengl_func_names); i++) {
    if (!(((void **)&opengl_funcs.gl)[i] =
              dlsym(opengl_handle, opengl_func_names[i]))) {
...
#define REDIRECT(func)                                                         \
  do {                                                                         \
    p##func = opengl_funcs.gl.p_##func;                                        \
    opengl_funcs.gl.p_##func = w##func;                                        \
  } while (0)
  REDIRECT(glFinish);
  REDIRECT(glFlush);
...
#define LOAD_FUNCPTR(f)                                                        \
  do                                                                           \
    if ((p##f = (void *)pglXGetProcAddressARB((const unsigned char *)#f)) ==   \
        NULL) {                                                                \
      ERR("%s not found in libGL, disabling OpenGL.\n", #f);                   \
      goto failed;                                                             \
    }                                                                          \
...
#undef LOAD_FUNCPTR
  ...
  if (wine_x11forceglx == -1)
    wine_x11forceglx =
        getenv("WINE_X11FORCEGLX") && atoi(getenv("WINE_X11FORCEGLX"));

  if (XQueryExtension(gdi_display, "GLX", &glx_opcode, &event_base,
                      &error_base) ||
      wine_x11forceglx) {
  ...
  /* In case of GLX you have direct and indirect rendering. Most of the time
   * direct rendering is used as in general only that is hardware accelerated.
   * In some cases like in case of remote X indirect rendering is used.
   *
   * The main problem for our OpenGL code is that we need certain GLX calls but
   * their presence depends on the reported GLX client / server version and on
   * the client / server extension list. Those don't have to be the same.
   *
   * In general the server GLX information lists the capabilities in case of
   * indirect rendering. When direct rendering is used, the OpenGL client
   * library is responsible for which GLX calls are available and in that case
   * the client GLX informat can be used. OpenGL programs should use the
   * 'intersection' of both sets of information which is advertised in the GLX
   * version/extension list. When a program does this it works for certain for
   * both direct and indirect rendering.
   *
   * The problem we are having in this area is that ATI's Linux drivers are
   * broken. For some reason they haven't added some very important GLX
   * extensions like GLX_SGIX_fbconfig to their client extension list which
   * causes this extension not to be listed. (Wine requires this extension). ATI
   * advertises a GLX client version of 1.3 which implies that this fbconfig
   * extension among pbuffers is around.
   *
   * In order to provide users of Ati's proprietary drivers with OpenGL support,
   * we need to detect the ATI drivers and from then on use GLX client
   * information for them.
   */

  if (glxRequireVersion(3)) {
    pglXChooseFBConfig =
        pglXGetProcAddressARB((const GLubyte *)"glXChooseFBConfig");
    pglXGetFBConfigAttrib =
        pglXGetProcAddressARB((const GLubyte *)"glXGetFBConfigAttrib");
    pglXGetVisualFromFBConfig =
        pglXGetProcAddressARB((const GLubyte *)"glXGetVisualFromFBConfig");
    pglXQueryDrawable =
        pglXGetProcAddressARB((const GLubyte *)"glXQueryDrawable");
  } else if (has_extension(glxExtensions, "GLX_SGIX_fbconfig")) {
    pglXChooseFBConfig =
        pglXGetProcAddressARB((const GLubyte *)"glXChooseFBConfigSGIX");
    pglXGetFBConfigAttrib =
        pglXGetProcAddressARB((const GLubyte *)"glXGetFBConfigAttribSGIX");
    pglXGetVisualFromFBConfig =
        pglXGetProcAddressARB((const GLubyte *)"glXGetVisualFromFBConfigSGIX");

    /* The mesa libGL client library seems to forward glXQueryDrawable to the
     * Xserver, so only enable this function when the Xserver understand GLX 1.3
     * or newer
     */
    ...
    ERR(" glx_version is %s and GLX_SGIX_fbconfig extension is unsupported. "
        "Expect problems.\n",
        pglXQueryServerString(gdi_display, DefaultScreen(gdi_display),
                              GLX_VERSION));
  }

  if (has_extension(glxExtensions, "GLX_MESA_copy_sub_buffer")) {
    pglXCopySubBufferMESA =
        pglXGetProcAddressARB((const GLubyte *)"glXCopySubBufferMESA");
  }
  ...
  if (has_extension(glxExtensions, "GLX_MESA_query_renderer")) {
    ...
    pglXQueryRendererIntegerMESA =
        pglXGetProcAddressARB((const GLubyte *)"glXQueryRendererIntegerMESA");
    pglXQueryRendererStringMESA =
        pglXGetProcAddressARB((const GLubyte *)"glXQueryRendererStringMESA");
  }

  if (has_extension(glxExtensions, "GLX_OML_sync_control")) {
    pglXWaitForSbcOML =
        pglXGetProcAddressARB((const GLubyte *)"glXWaitForSbcOML");
    pglXSwapBuffersMscOML =
        pglXGetProcAddressARB((const GLubyte *)"glXSwapBuffersMscOML");
  }
...
static const char *debugstr_fbconfig(GLXFBConfig fbconfig) {
  ...
  return wine_dbg_sprintf("fbconfig %#x visual id %#x drawable type %#x", id,
                          visual, drawable);
}

static int get_render_type_from_fbconfig(Display *display,
                                         GLXFBConfig fbconfig) {
...
static BOOL check_fbconfig_bitmap_capability(Display *display,
                                             GLXFBConfig fbconfig) {
  int dbuf, value;
  ...
  /* Windows only supports bitmap rendering on single buffered formats, further
   * the fbconfig needs to have the GLX_PIXMAP_BIT set. */
...
static void init_pixel_formats(Display *display) {
  ...
  /* Bitmap rendering on Windows implies the use of the Microsoft GDI software
   * renderer. Further most GLX drivers only offer pixmap rendering using
   * indirect rendering (except for modern drivers which support 'AIGLX' /
   * composite). Indirect rendering can indicate software rendering (on Nvidia
   * it is hw accelerated) Since bitmap rendering implies the use of software
   * rendering we can safely use indirect rendering for bitmaps.
   *
   * Below we count the number of formats which are suitable for bitmap
   * rendering. Windows restricts bitmap rendering to single buffered formats.
   */
  ...
  /* Fill the pixel format list. Put onscreen formats at the top and offscreen
   * ones at the bottom. Do this as GLX doesn't guarantee that the list is
   * sorted */
  for (run = 0; run < 2; run++) {
    for (i = 0; i < nCfgs; i++) {
      ...
      /* The first run we only add onscreen formats (ones which have an
       * associated X Visual). The second run we only set offscreen formats. */
      if (!run && visinfo) {
        /* We implement child window rendering using offscreen buffers (using
         * composite or an XPixmap). The contents is copied to the destination
         * using XCopyArea. For the copying to work the depth of the source and
         * destination window should be the same. In general this should not be
         * a problem for OpenGL as drivers only advertise formats with a similar
         * depth (or no depth). As of the introduction of composition managers
         * at least Nvidia now also offers ARGB visuals with a depth of 32 in
         * addition to the default 24 bit. In order to prevent BadMatch errors
         * we only list formats with the same depth. */
        ...
        list[size].render_type =
            get_render_type_from_fbconfig(display, cfgs[i]);
        list[size].dwFlags = 0;
        size++;
        onscreen_size++;

        /* Clone a format if it is bitmap capable for indirect rendering to
         * bitmaps */
        if (check_fbconfig_bitmap_capability(display, cfgs[i])) {
          TRACE("Found bitmap capable format FBCONFIG_ID 0x%x corresponding to "
                "iPixelFormat %d at GLX index %d\n",
                fmt_id, size + 1, i);
          list[size].fbconfig = cfgs[i];
          list[size].visual = visinfo;
          ...
          list[size].render_type =
              get_render_type_from_fbconfig(display, cfgs[i]);
          list[size].dwFlags =
              PFD_DRAW_TO_BITMAP | PFD_SUPPORT_GDI | PFD_GENERIC_FORMAT;
          size++;
          onscreen_size++;
        }
      } else if (run && !visinfo) {
        int window_drawable = 0;
        pglXGetFBConfigAttrib(gdi_display, cfgs[i], GLX_DRAWABLE_TYPE,
                              &window_drawable);

        /* Recent Nvidia drivers and DRI drivers offer window drawable formats
         * without a visual. This are formats like 16-bit rgb on a 24-bit
         * desktop. In order to support these formats onscreen we would have to
         * use glXCreateWindow instead of XCreateWindow. Further it will likely
         * make our child window opengl rendering more complicated since likely
         * you can't use XCopyArea on a GLX Window. For now ignore fbconfigs
         * which are window drawable but lack a visual. */
        if (window_drawable & GLX_WINDOW_BIT) {
          TRACE("Skipping FBCONFIG_ID 0x%x as an offscreen format because it "
                "is window_drawable\n",
                fmt_id);
          continue;
        }
...
/* GLX can advertise dozens of different pixelformats including offscreen and
 * onscreen ones. In our WGL implementation we only support a subset of these
 * formats namely the format of Wine's main visual and offscreen formats (if
 * they are available). This function converts a WGL format to its corresponding
 * GLX one.
 */
static const struct glx_pixel_format *
get_pixel_format(Display *display, int iPixelFormat, BOOL AllowOffscreen) {
...
static void release_gl_drawable(struct gl_drawable *gl) {
  if (!gl)
    return;
  if (InterlockedDecrement(&gl->ref))
    return;
  switch (gl->type) {
  case DC_GL_WINDOW:
...
static void mark_drawable_dirty(struct gl_drawable *old,
                                struct gl_drawable *new) {
...
static inline void sync_context(struct wgl_context *context) {
  ...
  if (refresh) {
    if (glxRequireVersion(3))
      ...
      pglXMakeCurrent(gdi_display, context->drawables[0]->drawable,
                      context->ctx);
...
static struct gl_drawable *get_gl_drawable(HWND hwnd, HDC hdc) {
  ...
  pthread_mutex_lock(&context_mutex);
  if (hwnd &&
      !XFindContext(gdi_display, (XID)hwnd, gl_hwnd_context, (char **)&gl))
    gl = grab_gl_drawable(gl);
  else if (hdc && !XFindContext(gdi_display, (XID)hdc, gl_pbuffer_context,
                                (char **)&gl))
    gl = grab_gl_drawable(gl);
  else
    gl = NULL;
...
static GLXContext create_glxcontext(Display *display,
 ...
 *              create_gl_drawable
 */
static struct gl_drawable *
create_gl_drawable(HWND hwnd, const struct glx_pixel_format *format,
                   BOOL known_child, BOOL mutable_pf) {
  ...
  if (!needs_offscreen_rendering(hwnd, known_child)) {
    ...
    if (gl->window)
      gl->drawable =
          pglXCreateWindow(gdi_display, gl->format->fbconfig, gl->window, NULL);
    ...
    if (gl->window) {
      ...
      gl->drawable =
          pglXCreateWindow(gdi_display, gl->format->fbconfig, gl->window, NULL);
      pXCompositeRedirectWindow(gdi_display, gl->window,
      ...
      gl->hdc_dst =
          NtGdiOpenDCW(&device_str, NULL, NULL, 0, TRUE, NULL, NULL, NULL);
      gl->hdc_src =
          NtGdiOpenDCW(&device_str, NULL, NULL, 0, TRUE, NULL, NULL, NULL);
    ...
    gl->pixmap =
        XCreatePixmap(gdi_display, root_window, width, height, visual->depth);
    if (gl->pixmap) {
      gl->drawable =
          pglXCreatePixmap(gdi_display, gl->format->fbconfig, gl->pixmap, NULL);
      if (!gl->drawable)
        XFreePixmap(gdi_display, gl->pixmap);
  ...
  XSaveContext(gdi_display, (XID)hwnd, gl_hwnd_context,
 ...
 *              set_win_format
 */
static BOOL set_win_format(HWND hwnd, const struct glx_pixel_format *format,
                           BOOL internal) {
  ...
  TRACE("created GL drawable %lx for win %p %s\n", gl->drawable, hwnd,
        debugstr_fbconfig(format->fbconfig));
...
static BOOL set_pixel_format(HDC hdc, int format, BOOL internal) {
   ...
   * Switching pixel format involves drawable recreation and is much more
   * expensive than blitting from backup context. */
...
void sync_gl_drawable(HWND hwnd, BOOL known_child) {
  ...
  case DC_GL_CHILD_WIN:
    ...
    /* fall through */
  case DC_GL_PIXMAP_WIN:
    if (!(new = create_gl_drawable(hwnd, old->format, known_child,
                                   old->mutable_pf)))
      break;
 ...
 */
static int describe_pixel_format(int iPixelFormat,
  ...
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_DRAWABLE_TYPE,
                            &drawable_type))
    return 0;
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_RENDER_TYPE,
                            &render_type))
    return 0;
  ...
  pf->pfd.dwFlags |=
      fmt->dwFlags & (PFD_GENERIC_FORMAT | PFD_GENERIC_ACCELERATED);

  if (!(pf->pfd.dwFlags & PFD_GENERIC_FORMAT))
    pf->pfd.dwFlags |= PFD_SUPPORT_COMPOSITION;
  ...
  if (value)
    pf->pfd.dwFlags |= PFD_STEREO;

  /* Pixel type */
  if (render_type & GLX_RGBA_BIT)
    pf->pfd.iPixelType = PFD_TYPE_RGBA;
  else
    pf->pfd.iPixelType = PFD_TYPE_COLORINDEX;
  ...
  if (!has_swap_method)
    pf->swap_method = WGL_SWAP_EXCHANGE_ARB;
  else if (!pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                                  GLX_SWAP_METHOD_OML, &value)) {
    switch (value) {
    case GLX_SWAP_EXCHANGE_OML:
      pf->swap_method = WGL_SWAP_EXCHANGE_ARB;
      break;
    case GLX_SWAP_COPY_OML:
      pf->swap_method = WGL_SWAP_COPY_ARB;
      break;
    case GLX_SWAP_UNDEFINED_OML:
      pf->swap_method = WGL_SWAP_UNDEFINED_ARB;
      break;
    default: {
  ...
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_TRANSPARENT_TYPE,
                            &value))
    pf->transparent = -1;
  else
    pf->transparent = value != GLX_NONE;

  if (render_type & GLX_RGBA_BIT)
    pf->pixel_type = WGL_TYPE_RGBA_ARB;
  else if (render_type & GLX_COLOR_INDEX_BIT)
    pf->pixel_type = WGL_TYPE_COLORINDEX_ARB;
  else if (render_type & GLX_RGBA_FLOAT_BIT)
    pf->pixel_type = WGL_TYPE_RGBA_FLOAT_ATI;
  else if (render_type & GLX_RGBA_FLOAT_ATI_BIT)
    pf->pixel_type = WGL_TYPE_RGBA_FLOAT_ATI;
  else if (render_type & GLX_RGBA_UNSIGNED_FLOAT_BIT_EXT)
    pf->pixel_type = WGL_TYPE_RGBA_UNSIGNED_FLOAT_EXT;
  else {
  ...
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_MAX_PBUFFER_PIXELS,
                            &value))
    value = -1;
  pf->max_pbuffer_pixels = value;
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_MAX_PBUFFER_WIDTH,
                            &value))
    value = -1;
  pf->max_pbuffer_width = value;
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_MAX_PBUFFER_HEIGHT,
                            &value))
    value = -1;
  pf->max_pbuffer_height = value;

  if (!pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                             GLX_TRANSPARENT_RED_VALUE, &value)) {
    pf->transparent_red_value_valid = GL_TRUE;
    pf->transparent_red_value = value;
  }
  if (!pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                             GLX_TRANSPARENT_GREEN_VALUE, &value)) {
    pf->transparent_green_value_valid = GL_TRUE;
    pf->transparent_green_value = value;
  }
  if (!pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                             GLX_TRANSPARENT_BLUE_VALUE, &value)) {
    pf->transparent_blue_value_valid = GL_TRUE;
    pf->transparent_blue_value = value;
  }
  if (!pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                             GLX_TRANSPARENT_ALPHA_VALUE, &value)) {
    pf->transparent_alpha_value_valid = GL_TRUE;
    pf->transparent_alpha_value = value;
  }
  if (!pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                             GLX_TRANSPARENT_INDEX_VALUE, &value)) {
    pf->transparent_index_value_valid = GL_TRUE;
    pf->transparent_index_value = value;
  }

  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_SAMPLE_BUFFERS_ARB,
                            &value))
    value = -1;
  pf->sample_buffers = value;
  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_SAMPLES_ARB,
                            &value))
    value = -1;
  pf->samples = value;

  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig,
                            GLX_FRAMEBUFFER_SRGB_CAPABLE_EXT, &value))
    value = -1;
  pf->framebuffer_srgb_capable = value;

  pf->bind_to_texture_rgb = pf->bind_to_texture_rgba =
      use_render_texture_emulation && render_type != GLX_COLOR_INDEX_BIT &&
      (drawable_type & GLX_PBUFFER_BIT);
  pf->bind_to_texture_rectangle_rgb = pf->bind_to_texture_rectangle_rgba =
      GL_FALSE;

  if (pglXGetFBConfigAttrib(gdi_display, fmt->fbconfig, GLX_FLOAT_COMPONENTS_NV,
                            &value))
    value = -1;
  pf->float_components = value;

  if (TRACE_ON(wgl))
    dump_PIXELFORMATDESCRIPTOR(&pf->pfd);
...
static int glxdrv_wglGetPixelFormat(HDC hdc) {
  ...
  if ((gl = get_gl_drawable(NULL, hdc))) {
     ...
     * As has been verified on Windows GetPixelFormat doesn't fail but returns
     * iPixelFormat=1. */
    if (!is_onscreen_pixel_format(ret))
      ret = 1;
    release_gl_drawable(gl);
  }
  TRACE("%p -> %d\n", hdc, ret);
 ...
 */
static BOOL glxdrv_wglSetPixelFormat(HDC hdc, int iPixelFormat,
 ...
 */
static BOOL glxdrv_wglCopyContext(struct wgl_context *src,
  ...
  if (X11DRV_check_error()) {
    ...
    if (!once++) {
      ERR("glXCopyContext failed. glXCopyContext() for direct rendering "
          "contexts not "
...
static PROC glxdrv_wglGetProcAddress(LPCSTR lpszProc) {
  if (!strncmp(lpszProc, "wgl", 3))
    return NULL;
  return pglXGetProcAddressARB((const GLubyte *)lpszProc);
}

static void set_context_drawables(struct wgl_context *ctx,
...
static BOOL glxdrv_wglMakeCurrent(HDC hdc, struct wgl_context *ctx) {
  ...
  if ((gl = get_gl_drawable(NtUserWindowFromDC(hdc), hdc))) {
    ...
    TRACE("hdc %p drawable %lx fmt %p ctx %p %s\n", hdc, gl->drawable,
          gl->format, ctx->ctx, debugstr_fbconfig(gl->format->fbconfig));
 ...
 */
static BOOL X11DRV_wglMakeContextCurrentARB(HDC draw_hdc, HDC read_hdc,
                                            struct wgl_context *ctx) {
  BOOL ret = FALSE;
 ...
 */
static BOOL glxdrv_wglShareLists(struct wgl_context *org,
                                 struct wgl_context *dest) {
  struct wgl_context *keep, *clobber;

  TRACE("(%p, %p)\n", org, dest);

  /* Sharing of display lists works differently in GLX and WGL. In case of GLX
   * it is done at context creation time but in case of WGL it is done using
   * wglShareLists. In the past we tried to emulate wglShareLists by delaying
   * GLX context creation until either a wglMakeCurrent or wglShareLists. This
   * worked fine for most apps but it causes issues for OpenGL 3 because there
   * wglCreateContextAttribsARB can fail in a lot of cases, so there delaying
   * context creation doesn't work.
   *
   * The new approach is to create a GLX context in wglCreateContext /
   * wglCreateContextAttribsARB and when a program requests sharing we recreate
   * the destination or source context if it hasn't been made current and it
  ...
  TRACE("re-created context (%p) for Wine context %p (%s) sharing lists with "
        "ctx %p (%s)\n",
...
static void present_gl_drawable(HWND hwnd, HDC hdc, struct gl_drawable *gl,
                                BOOL flush) {
  ...
  NtUserGetClientRect(hwnd, &rect_dst,
                      NtUserGetWinMonitorDpi(hwnd, MDT_DEFAULT));
  NtUserMapWindowPoints(hwnd, toplevel, (POINT *)&rect_dst, 2,
                        NtUserGetWinMonitorDpi(hwnd, MDT_DEFAULT));
  ...
  if (get_dc_drawable(gl->hdc_dst, &rect) != window ||
      !EqualRect(&rect, &rect_dst))
    set_dc_drawable(gl->hdc_dst, window, &rect_dst, ClipByChildren);
  if (region)
    NtGdiExtSelectClipRgn(gl->hdc_dst, region, RGN_COPY);

  NtGdiStretchBlt(gl->hdc_dst, 0, 0, rect_dst.right - rect_dst.left,
                  rect_dst.bottom - rect_dst.top, gl->hdc_src, 0, 0,
                  gl->rect.right, gl->rect.bottom, SRCCOPY, 0);
...
static const GLubyte *wglGetString(GLenum name) {
  if (name == GL_EXTENSIONS && glExtensions)
    return (const GLubyte *)glExtensions;
 ...
 */
static struct wgl_context *
X11DRV_wglCreateContextAttribsARB(HDC hdc, struct wgl_context *hShareContext,
                                  const int *attribList) {
  struct wgl_context *ret;
  struct gl_drawable *gl;
  int err = 0;
  ...
  if ((ret = calloc(1, sizeof(*ret)))) {
    ...
    ret->ctx = create_glxcontext(gdi_display, ret,
                                 hShareContext ? hShareContext->ctx : NULL);
    XSync(gdi_display, False);
    if ((err = X11DRV_check_error()) || !ret->ctx) {
      /* In the future we should convert the GLX error to a win32 one here if
       * needed */
 ...
 */
static struct wgl_pbuffer *X11DRV_wglCreatePbufferARB(HDC hdc, int iPixelFormat,
  ...
  /* Convert the WGL pixelformat to a GLX format, if it fails then the format is
   * invalid */
  fmt = get_pixel_format(gdi_display, iPixelFormat, TRUE /* Offscreen */);
  if (!fmt) {
...
static HDC X11DRV_wglGetPbufferDCARB(struct wgl_pbuffer *object) {
  ...
  NtGdiExtEscape(hdc, NULL, 0, X11DRV_ESCAPE, sizeof(escape), (LPSTR)&escape, 0,
                 NULL);
 ...
 */
static BOOL X11DRV_wglQueryPbufferARB(struct wgl_pbuffer *object,
                                      int iAttribute, int *piValue) {
  TRACE("(%p, 0x%x, %p)\n", object, iAttribute, piValue);

  switch (iAttribute) {
  case WGL_PBUFFER_WIDTH_ARB:
    pglXQueryDrawable(gdi_display, object->gl->drawable, GLX_WIDTH,
                      (unsigned int *)piValue);
    break;
  case WGL_PBUFFER_HEIGHT_ARB:
    pglXQueryDrawable(gdi_display, object->gl->drawable, GLX_HEIGHT,
                      (unsigned int *)piValue);
    break;
 ...
 */
static BOOL X11DRV_wglSetPbufferAttribARB(struct wgl_pbuffer *object,
                                          const int *piAttribList) {
...
static BOOL X11DRV_wglBindTexImageARB(struct wgl_pbuffer *object, int iBuffer) {
  ...
  if (use_render_texture_emulation) {
    ...
    /* Our render_texture emulation is basic and lacks some features (1D/Cube
       support). This is mostly due to lack of demos/games using them. Further
       the use of glReadPixels isn't ideal performance wise but I wasn't able to
       get other ways working.
    */
    if (!initialized) {
    ...
    opengl_funcs.gl.p_glGetIntegerv(object->texture_bind_target,
                                    &prev_binded_texture);

    /* Switch to our pbuffer */
    pglXMakeCurrent(gdi_display, object->gl->drawable, object->tmp_context);

    /* Make sure that the prev_binded_texture is set as the current texture
     * state isn't shared between contexts. After that copy the pbuffer texture
     * data. */
    opengl_funcs.gl.p_glBindTexture(object->texture_target,
                                    prev_binded_texture);
    opengl_funcs.gl.p_glCopyTexImage2D(object->texture_target, 0,
                                       object->use_render_texture, 0, 0,
 ...
 */
static BOOL X11DRV_wglReleaseTexImageARB(struct wgl_pbuffer *object,
                                         int iBuffer) {
  GLboolean ret = GL_FALSE;
 ...
 * This is a WINE-specific wglSetPixelFormat which can set the pixel format
 * multiple times.
 */
...
static BOOL X11DRV_wglQueryCurrentRendererIntegerWINE(GLenum attribute,
                                                      GLuint *value) {
...
static BOOL X11DRV_wglQueryRendererIntegerWINE(HDC dc, GLint renderer,
                                               GLenum attribute,
                                               GLuint *value) {
  return pglXQueryRendererIntegerMESA(gdi_display, DefaultScreen(gdi_display),
                                      renderer, attribute, value);
}

static const char *X11DRV_wglQueryRendererStringWINE(HDC dc, GLint renderer,
                                                     GLenum attribute) {
  return pglXQueryRendererStringMESA(gdi_display, DefaultScreen(gdi_display),
                                     renderer, attribute);
}
...
static void X11DRV_WineGL_LoadExtensions(void) {
  ...
  if (has_extension(glxExtensions, "GLX_ARB_create_context")) {
    ...
    opengl_funcs.ext.p_wglCreateContextAttribsARB =
        X11DRV_wglCreateContextAttribsARB;
  ...
  opengl_funcs.ext.p_wglGetExtensionsStringARB =
      X11DRV_wglGetExtensionsStringARB;

  if (glxRequireVersion(3)) {
    register_extension("WGL_ARB_make_current_read");
    opengl_funcs.ext.p_wglGetCurrentReadDCARB = (void *)1; /* never called */
    opengl_funcs.ext.p_wglMakeContextCurrentARB =
        X11DRV_wglMakeContextCurrentARB;
  }
  ...
  opengl_funcs.ext.p_wglGetPixelFormatAttribfvARB =
      (void *)1; /* never called */
  opengl_funcs.ext.p_wglGetPixelFormatAttribivARB =
      (void *)1; /* never called */
  ...
  /* Support WGL_ARB_render_texture when there's support or pbuffer based
   * emulation */
  if (has_extension(glxExtensions, "GLX_ARB_render_texture") ||
    ...
    /* Again there's no GLX equivalent for this extension, so depend on the
     * required GL extension */
  ...
  opengl_funcs.ext.p_wglGetExtensionsStringEXT =
      X11DRV_wglGetExtensionsStringEXT;

  /* Load this extension even when it isn't backed by a GLX extension because it
   * is has been around for ages. Games like Call of Duty and K.O.T.O.R. rely on
   * it. Further our emulation is good enough. */
  ...
  /* The OpenGL extension GL_NV_vertex_array_range adds wgl/glX functions which
   * aren't exported as 'real' wgl/glX extensions. */
  ...
  /* In WineD3D we need the ability to set the pixel format more than once (e.g.
   * after a device reset). The default wglSetPixelFormat doesn't allow this, so
   * add our own which allows it.
   */
  ...
  if (has_extension(glxExtensions, "GLX_MESA_query_renderer")) {
    ...
    opengl_funcs.ext.p_wglQueryCurrentRendererIntegerWINE =
        X11DRV_wglQueryCurrentRendererIntegerWINE;
    opengl_funcs.ext.p_wglQueryCurrentRendererStringWINE =
        X11DRV_wglQueryCurrentRendererStringWINE;
    opengl_funcs.ext.p_wglQueryRendererIntegerWINE =
        X11DRV_wglQueryRendererIntegerWINE;
    opengl_funcs.ext.p_wglQueryRendererStringWINE =
...
static BOOL glxdrv_wglSwapBuffers(HDC hdc) {
  ...
  case DC_GL_PIXMAP_WIN:
    ...
    if (ctx && pglXCopySubBufferMESA) {
       ...
       * copying */
      pglFlush();
      pglXCopySubBufferMESA(gdi_display, gl->drawable, 0, 0, gl->rect.right,
                            gl->rect.bottom);
      break;
    }
...
struct opengl_funcs *get_glx_driver(UINT version) { return NULL; }
