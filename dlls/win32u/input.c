...
static const WCHAR keyboard_layouts_keyW[] = {
    '\\', 'R', 'e', 'g',  'i', 's', 't', 'r', 'y',  '\\', 'M',  'a', 'c', 'h',
    'i',  'n', 'e', '\\', 'S', 'y', 's', 't', 'e',  'm',  '\\', 'C', 'u', 'r',
    'r',  'e', 'n', 't',  'C', 'o', 'n', 't', 'r',  'o',  'l',  'S', 'e', 't',
    '\\', 'C', 'o', 'n',  't', 'r', 'o', 'l', '\\', 'K',  'e',  'y', 'b', 'o',
    'a',  'r', 'd', ' ',  'L', 'a', 'y', 'o', 'u',  't',  's'};
static const WCHAR escW[] = {'E', 's', 'c', 0};
static const WCHAR backspaceW[] = {'B', 'a', 'c', 'k', 's',
                                   'p', 'a', 'c', 'e', 0};
...
static const WCHAR scroll_lockW[] = {'S', 'c', 'r', 'o', 'l', 'l',
                                     ' ', 'L', 'o', 'c', 'k', 0};
...
static const WCHAR num_enterW[] = {'N', 'u', 'm', ' ', 'E',
                                   'n', 't', 'e', 'r', 0};
static const WCHAR right_ctrlW[] = {'R', 'i', 'g', 'h', 't', ' ',
                                    'C', 't', 'r', 'l', 0};
static const WCHAR num_divW[] = {'N', 'u', 'm', ' ', '/', 0};
static const WCHAR prnt_scrnW[] = {'P', 'r', 'n', 't', ' ',
                                   'S', 'c', 'r', 'n', 0};
static const WCHAR right_altW[] = {'R', 'i', 'g', 'h', 't',
                                   ' ', 'A', 'l', 't', 0};
...
static const WCHAR page_downW[] = {'P', 'a', 'g', 'e', ' ',
                                   'D', 'o', 'w', 'n', 0};
static const WCHAR insertW[] = {'I', 'n', 's', 'e', 'r', 't', 0};
static const WCHAR deleteW[] = {'D', 'e', 'l', 'e', 't', 'e', 0};
static const WCHAR zerozeroW[] = {'<', '0', '0', '>', 0};
static const WCHAR helpW[] = {'H', 'e', 'l', 'p', 0};
static const WCHAR left_windowsW[] = {'L', 'e', 'f', 't', ' ', 'W', 'i',
                                      'n', 'd', 'o', 'w', 's', 0};
static const WCHAR right_windowsW[] = {'R', 'i', 'g', 'h', 't', ' ', 'W',
                                       'i', 'n', 'd', 'o', 'w', 's', 0};
static const WCHAR applicationW[] = {'A', 'p', 'p', 'l', 'i', 'c',
                                     'a', 't', 'i', 'o', 'n', 0};
...
static const VK_TO_WCHAR_TABLE vk_to_wchar_table[] = {
    {.pVkToWchars = (VK_TO_WCHARS1 *)vk_to_wchars3,
     .nModifications = 3,
     .cbSize = sizeof(vk_to_wchars3[0])},
    {.pVkToWchars = (VK_TO_WCHARS1 *)vk_to_wchars4,
     .nModifications = 4,
     .cbSize = sizeof(vk_to_wchars4[0])},
    {.pVkToWchars = (VK_TO_WCHARS1 *)vk_to_wchars2,
     .nModifications = 2,
     .cbSize = sizeof(vk_to_wchars2[0])},
    {.pVkToWchars = (VK_TO_WCHARS1 *)vk_to_wchars1,
     .nModifications = 1,
     .cbSize = sizeof(vk_to_wchars1[0])},
    {0},
};
...
static const USHORT vsc_to_vk[] = {T00,
                                   T01,
                                   T02,
                                   T03,
                                   T04,
                                   ...
                                   T46 | KBDMULTIVK,
                                   T47 | KBDNUMPAD | KBDSPECIAL,
                                   T48 | KBDNUMPAD | KBDSPECIAL,
                                   T49 | KBDNUMPAD | KBDSPECIAL,
                                   T4A,
                                   T4B | KBDNUMPAD | KBDSPECIAL,
                                   T4C | KBDNUMPAD | KBDSPECIAL,
                                   T4D | KBDNUMPAD | KBDSPECIAL,
                                   T4E,
                                   T4F | KBDNUMPAD | KBDSPECIAL,
                                   T50 | KBDNUMPAD | KBDSPECIAL,
                                   T51 | KBDNUMPAD | KBDSPECIAL,
                                   T52 | KBDNUMPAD | KBDSPECIAL,
                                   T53 | KBDNUMPAD | KBDSPECIAL,
                                   T54,
                                   T55,
                                   T56,
                                   T57,
...
static void kbd_tables_init_vsc2vk(const KBDTABLES *tables,
                                   USHORT vsc2vk[0x300]) {
  const VSC_VK *entry;
  WORD vsc;
...
static void kbd_tables_init_vk2char(const KBDTABLES *tables,
                                    BYTE vk2char[0x100]) {
  ...
  for (table = tables->pVkToWcharTable; table->pVkToWchars; table++) {
    for (entry = table->pVkToWchars; entry->VirtualKey;
         entry = NEXT_ENTRY(table, entry)) {
...
static UINT kbd_tables_get_mod_bits(const KBDTABLES *tables, UINT mod) {
  ...
  for (bits = 0; bits <= mods->wMaxModBits; ++bits)
    if (mods->ModNumber[bits] == mod)
      return bits;

  return -1;
}

static UINT kbd_tables_get_mod_num(const KBDTABLES *tables, const BYTE *state,
                                   BOOL caps) {
  const MODIFIERS *mods = tables->pCharModifiers;
  ...
  for (entry = mods->pVkToBit; entry->Vk; ++entry)
    if (state[entry->Vk] & 0x80)
      bits |= entry->ModBits;
  if (caps)
    bits |= KBDSHIFT;

  if (bits > mods->wMaxModBits)
    return -1;
...
static WORD kbd_tables_wchar_to_vkey(const KBDTABLES *tables, WCHAR wch) {
  ...
  for (table = tables->pVkToWcharTable; table->pVkToWchars; table++) {
    for (entry = table->pVkToWchars; entry->VirtualKey;
      ...
      for (mod = 0; mod < table->nModifications; ++mod) {
        if (entry->wch[mod] == WCH_NONE || entry->wch[mod] != wch)
          continue;
  ...
  if (wch >= 0x0001 && wch <= 0x001a)
    return (0x200) | ('A' + wch - 1); /* CTRL + A-Z */
  return wch >= 0x0080 ? -1 : 0;
}

static WCHAR kbd_tables_vkey_to_wchar(const KBDTABLES *tables, UINT vkey,
                                      const BYTE *state) {
  ...
  if (ctrl && alt && !(tables->fLocaleFlags & KLLF_ALTGR))
    return WCH_NONE;
  if (!ctrl && vkey == VK_ESCAPE)
    return VK_ESCAPE;
  if (ctrl && !alt) {
    if (vkey >= 'A' && vkey <= 'Z')
      return vkey - 'A' + 1;
  ...
  if (caps)
    caps_mod = kbd_tables_get_mod_num(tables, state, TRUE);

  for (table = tables->pVkToWcharTable; table->pVkToWchars; table++) {
    if (table->nModifications <= mod)
      continue;
    for (entry = table->pVkToWchars; entry->VirtualKey;
         entry = NEXT_ENTRY(table, entry)) {
      if (entry->VirtualKey != vkey)
        continue;
      /* SGCAPS attribute may be set on entries where VK_CAPITAL and VK_SHIFT
       * behave differently. The entry corresponds to the mapping when Caps Lock
       * is on, and a second entry follows it with the mapping when Caps Lock is
       * off.
       */
      if ((entry->Attributes & SGCAPS) && !caps)
        entry = NEXT_ENTRY(table, entry);
      if ((entry->Attributes & CAPLOK) && table->nModifications > caps_mod)
        return entry->wch[caps_mod];
...
static void update_mouse_coords(INPUT *input) {
  ...
  if (input->mi.dwFlags & MOUSEEVENTF_ABSOLUTE) {
    ...
    if (abs(input->mi.dx) > accel[0]) {
      input->mi.dx *= 2;
      if (abs(input->mi.dx) > accel[1] && accel[2] == 2)
        input->mi.dx *= 2;
    }
    if (abs(input->mi.dy) > accel[0]) {
      input->mi.dy *= 2;
      if (abs(input->mi.dy) > accel[1] && accel[2] == 2)
        input->mi.dy *= 2;
    }
  }
}
...
SHORT WINAPI NtUserGetKeyState(INT vkey) {
  ...
  while ((status = get_shared_input(GetCurrentThreadId(), &lock, &input_shm)) ==
         STATUS_PENDING) {
    ret = !!input_shm
                ->keystate_lock; /* needs a request for sync_input_keystate */
  ...
  if (!ret)
    SERVER_START_REQ(get_key_state) {
      req->key = vkey;
      if (!wine_server_call(req))
        retval = (signed char)(reply->state & 0x81);
    }
  SERVER_END_REQ;
  TRACE("key (0x%x) -> %x\n", vkey, retval);
  return retval;
...
WORD WINAPI NtUserVkKeyScanEx(WCHAR chr, HKL layout) {
  ...
  if ((ret = user_driver->pVkKeyScanEx(chr, layout)) != -256)
    return ret;

  if (!(kbd_tables = user_driver->pKbdLayerDescriptor(layout)))
    kbd_tables = &kbdus_tables;
  ret = kbd_tables_wchar_to_vkey(kbd_tables, chr);
  if (kbd_tables != &kbdus_tables)
...
UINT WINAPI NtUserMapVirtualKeyEx(UINT code, UINT type, HKL layout) {
  ...
  if ((ret = user_driver->pMapVirtualKeyEx(code, type, layout)) != -1)
    return ret;
  ...
  case MAPVK_VK_TO_VSC:
    ...
    for (ret = 0; ret < ARRAY_SIZE(vsc2vk); ++ret)
      if ((vsc2vk[ret] & 0xff) == code)
        break;
    if (ret >= ARRAY_SIZE(vsc2vk))
      ret = 0;
  ...
  case MAPVK_VSC_TO_VK_EX:
    ...
    if (code & 0xe000)
      code -= 0xdf00;
    if (code >= ARRAY_SIZE(vsc2vk))
      ret = 0;
    else
      ret = vsc2vk[code] & 0xff;
...
USHORT map_scan_to_kbd_vkey(USHORT scan, HKL layout) {
  ...
  if ((vkey = user_driver->pMapVirtualKeyEx(scan, MAPVK_VSC_TO_VK_EX,
                                            layout)) != -1)
    return vkey;
...
INT WINAPI NtUserGetKeyNameText(LONG lparam, WCHAR *buffer, INT size) {
  ...
  if (!buffer || !size)
    return 0;
  if ((len = user_driver->pGetKeyNameText(lparam, buffer, size)) >= 0)
    return len;
  ...
  if (code < 0x100)
    key_name = kbd_tables->pKeyNames;
  else
    key_name = kbd_tables->pKeyNamesExt;
  while (key_name->vsc && key_name->vsc != (BYTE)code)
    key_name++;
...
INT WINAPI NtUserToUnicodeEx(UINT virt, UINT scan, const BYTE *state,
  ...
  if (!state || !size)
    return 0;
  if ((len = user_driver->pToUnicodeEx(virt, scan, state, str, size, flags,
                                       layout)) >= -1)
    return len;
  ...
  if (scan & 0x8000)
    ...
    str[0] = kbd_tables_vkey_to_wchar(kbd_tables, virt, state);
  if (size > 1)
    str[1] = 0;
...
HKL WINAPI NtUserActivateKeyboardLayout(HKL layout, UINT flags) {
  ...
  if (old_layout != layout) {
    ...
    if (ime_hwnd)
      send_message(ime_hwnd, WM_IME_INTERNAL, IME_INTERNAL_HKL_DEACTIVATE,
    ...
    if (HIWORD(layout) & 0x8000)
      ...
      translate_charset_info(ULongToPtr(data->idefaultansicodepage), &cs,
                             TCI_SRCCODEPAGE);
    ...
    if (ime_hwnd)
      send_message(ime_hwnd, WM_IME_INTERNAL, IME_INTERNAL_HKL_ACTIVATE,
...
UINT WINAPI NtUserGetKeyboardLayoutList(INT size, HKL *layouts) {
  ...
  if ((count = user_driver->pGetKeyboardLayoutList(size, layouts)) != ~0)
    return count;
  ...
  if ((hkey = reg_open_key(NULL, keyboard_layouts_keyW,
    ...
    while (!NtEnumerateKey(hkey, i++, KeyNodeInformation, key_info,
      ...
      if (!(subkey = reg_open_key(hkey, key_info->Name, key_info->NameLength)))
        continue;
      key_info->Name[key_info->NameLength / sizeof(WCHAR)] = 0;
      tmp = wcstoul(key_info->Name, NULL, 16);
      if (query_reg_ascii_value(subkey, "Layout Id", value_info,
                                sizeof(buffer)) &&
          value_info->Type == REG_SZ)
        tmp = 0xf000 |
              (wcstoul((const WCHAR *)value_info->Data, NULL, 16) & 0xfff);
      NtClose(subkey);

      tmp = MAKELONG(LOWORD(layout), LOWORD(tmp));
      if (layout == UlongToHandle(tmp))
        continue;
...
BOOL WINAPI NtUserGetKeyboardLayoutName(WCHAR *name) {
  ...
  if ((hkey = reg_open_key(NULL, keyboard_layouts_keyW,
    ...
    while (!NtEnumerateKey(hkey, i++, KeyNodeInformation, key,
      ...
      if (!(subkey = reg_open_key(hkey, key->Name, key->NameLength)))
        continue;
 ...
 */
int WINAPI NtUserGetMouseMovePointsEx(UINT size, MOUSEMOVEPOINT *ptin,
                                      MOUSEMOVEPOINT *ptout, int count,
                                      DWORD resolution) {
  ...
  if ((size != sizeof(MOUSEMOVEPOINT)) || (count < 0) ||
      (count > ARRAY_SIZE(positions))) {
    RtlSetLastWin32Error(ERROR_INVALID_PARAMETER);
    return -1;
  }
...
static WORD get_key_state(void) {
  ...
  if (get_system_metrics(SM_SWAPBUTTON)) {
    if (NtUserGetAsyncKeyState(VK_RBUTTON) & 0x80)
      ret |= MK_LBUTTON;
    if (NtUserGetAsyncKeyState(VK_LBUTTON) & 0x80)
      ret |= MK_RBUTTON;
  } else {
    if (NtUserGetAsyncKeyState(VK_LBUTTON) & 0x80)
      ret |= MK_LBUTTON;
  ...
  if (NtUserGetAsyncKeyState(VK_MBUTTON) & 0x80)
    ret |= MK_MBUTTON;
  if (NtUserGetAsyncKeyState(VK_SHIFT) & 0x80)
    ret |= MK_SHIFT;
  if (NtUserGetAsyncKeyState(VK_CONTROL) & 0x80)
    ret |= MK_CONTROL;
  if (NtUserGetAsyncKeyState(VK_XBUTTON1) & 0x80)
    ret |= MK_XBUTTON1;
  if (NtUserGetAsyncKeyState(VK_XBUTTON2) & 0x80)
    ret |= MK_XBUTTON2;
  return ret;
}
...
BOOL WINAPI NtUserTrackMouseEvent(TRACKMOUSEEVENT *info) {
  ...
  TRACE("size %u, flags %#x, hwnd %p, time %u\n", (int)info->cbSize,
        (int)info->dwFlags, info->hwndTrack, (int)info->dwHoverTime);
  ...
  if (info->dwFlags & ~(TME_CANCEL | TME_HOVER | TME_LEAVE | TME_NONCLIENT))
    FIXME("ignoring flags %#x\n",
          (int)info->dwFlags &
              ~(TME_CANCEL | TME_HOVER | TME_LEAVE | TME_NONCLIENT));
  ...
  if (info->dwFlags & TME_CANCEL) {
    if (tracking_info.info.hwndTrack == info->hwndTrack) {
      ...
      if (!(tracking_info.info.dwFlags & (TME_HOVER | TME_LEAVE))) {
        kill_system_timer(tracking_info.info.hwndTrack,
    ...
    if ((tracking_info.info.dwFlags & TME_LEAVE) &&
        tracking_info.info.hwndTrack != NULL)
      check_mouse_leave(hwnd, hittest);
    ...
    if (info->hwndTrack == hwnd) {
      ...
      NtUserSetSystemTimer(tracking_info.info.hwndTrack,
                           SYSTEM_TIMER_TRACK_MOUSE, hover_time);
    }
  }
...
BOOL set_capture_window(HWND hwnd, UINT gui_flags, HWND *prev_ret) {
  ...
  if (gui_flags & GUI_INMENUMODE)
    flags |= CAPTURE_MENU;
  if (gui_flags & GUI_INMOVESIZE)
    flags |= CAPTURE_MOVESIZE;
...
BOOL set_active_window(HWND hwnd, HWND *prev, BOOL mouse, BOOL focus,
  ...
  if (hwnd) {
    ...
    if (send_message(hwnd, WM_QUERYNEWPALETTE, 0, 0))
      send_message_timeout(HWND_BROADCAST, WM_PALETTEISCHANGING, (WPARAM)hwnd,
                           0, SMTO_ABORTIFHUNG, 2000, FALSE);
    if (!is_window(hwnd))
      return FALSE;
  }
  ...
  if (old_thread != new_thread) {
    ...
    if ((list = list_window_children(NULL, get_desktop_window(), NULL, 0))) {
      if (old_thread) {
        if (!new_active_thread_id)
          new_active_thread_id = new_thread;
  ...
  if (is_window(hwnd)) {
    send_message(hwnd, WM_NCACTIVATE, hwnd == NtUserGetForegroundWindow(),
                 (LPARAM)previous);
    send_message(
        hwnd, WM_ACTIVATE,
    ...
    if (NtUserGetAncestor(hwnd, GA_PARENT) == get_desktop_window())
      NtUserPostMessage(get_desktop_window(), WM_PARENTNOTIFY, WM_NCACTIVATE,
                        (LPARAM)hwnd);
  }
  ...
  if (focus) {
    ...
    if (hwnd == info.hwndActive) {
      if (!info.hwndFocus || !hwnd ||
          NtUserGetAncestor(info.hwndFocus, GA_ROOT) != hwnd)
...
HWND WINAPI NtUserSetActiveWindow(HWND hwnd) {
  ...
  if (hwnd) {
    ...
    if ((style & (WS_POPUP | WS_CHILD)) == WS_CHILD)
      return get_active_window(); /* Windows doesn't seem to return an error
                                     here */
  }

  if (!set_active_window(hwnd, &prev, FALSE, TRUE, 0))
    return 0;
  return prev;
}
...
HWND WINAPI NtUserSetFocus(HWND hwnd) {
  ...
  if (hwnd) {
    ...
    /* call hooks */
    if (call_hooks(WH_CBT, HCBT_SETFOCUS, (WPARAM)hwnd, (LPARAM)previous, 0))
      return 0;

    /* activate hwndTop if needed. */
    if (hwndTop != get_active_window()) {
      if (!set_active_window(hwndTop, NULL, FALSE, FALSE, 0))
        return 0;
      if (!is_window(hwnd))
...
BOOL set_foreground_window(HWND hwnd, BOOL mouse) {
  ...
  if (ret && previous != hwnd) {
    if (send_msg_old) /* old window belongs to other thread */
      NtUserMessageCall(previous, WM_WINE_SETACTIVEWINDOW, 0, new_thread_id, 0,
                        NtUserSendNotifyMessage, FALSE);
    else if (send_msg_new) /* old window belongs to us but new one to other
                              thread */
      ret = set_active_window(0, NULL, mouse, TRUE, new_thread_id);

    if (send_msg_new) /* new window belongs to other thread */
      NtUserMessageCall(hwnd, WM_WINE_SETACTIVEWINDOW, (WPARAM)hwnd, 0, 0,
                        NtUserSendNotifyMessage, FALSE);
...
static void display_caret(HWND hwnd, const RECT *r) {
  ...
  if (mem_dc) {
    ...
    NtGdiBitBlt(dc, r->left, r->top, r->right - r->left, r->bottom - r->top,
                mem_dc, 0, 0, SRCINVERT, 0, 0);
...
static unsigned int get_caret_registry_timeout(void) {
  char value_buffer[FIELD_OFFSET(KEY_VALUE_PARTIAL_INFORMATION,
                                 Data[11 * sizeof(WCHAR)])];
  ...
  if (query_reg_ascii_value(key, "CursorBlinkRate", value,
                            sizeof(value_buffer)))
 ...
 */
BOOL WINAPI NtUserCreateCaret(HWND hwnd, HBITMAP bitmap, int width,
                              int height) {
  ...
  if (bitmap && bitmap != (HBITMAP)1) {
    ...
    if (!NtGdiExtGetObjectW(bitmap, sizeof(bitmap_data), &bitmap_data))
      return FALSE;
    width = bitmap_data.bmWidth;
    height = bitmap_data.bmHeight;
    caret_bitmap =
        NtGdiCreateBitmap(bitmap_data.bmWidth, bitmap_data.bmHeight,
    ...
    if (!width)
      width = get_system_metrics(SM_CXBORDER);
    if (!height)
      height = get_system_metrics(SM_CYBORDER);
    ...
    if (dc) {
      ...
      if (mem_dc) {
        if ((caret_bitmap =
                 NtGdiCreateCompatibleBitmap(mem_dc, width, height))) {
          HBITMAP prev_bitmap = NtGdiSelectBitmap(mem_dc, caret_bitmap);
          SetRect(&r, 0, 0, width, height);
          fill_rect(mem_dc, &r,
                    GetStockObject(bitmap ? GRAY_BRUSH : WHITE_BRUSH));
          NtGdiSelectBitmap(mem_dc, prev_bitmap);
        }
        NtGdiDeleteObjectApp(mem_dc);
      }
  ...
  if (prev && !hidden) /* hide the previous one */
  {
    ...
    if (old_state)
      display_caret(prev, &r);
  }

  if (caret.bitmap)
    NtGdiDeleteObjectApp(caret.bitmap);
 ...
 */
UINT WINAPI NtUserGetCaretBlinkTime(void) { return caret.timeout; }
...
BOOL set_ime_composition_rect(HWND hwnd, RECT rect) {
  ...
  return user_driver->pSetIMECompositionRect(NtUserGetAncestor(hwnd, GA_ROOT),
                                             rect);
}
...
static BOOL is_captured_by_system(void) {
  ...
  return NtUserGetGUIThreadInfo(GetCurrentThreadId(), &info) &&
         info.hwndCapture && (info.flags & (GUI_INMOVESIZE | GUI_INMENUMODE));
}
...
BOOL clip_fullscreen_window(HWND hwnd, BOOL reset) {
  ...
  if (!get_window_rect(hwnd, &rect, dpi))
    return FALSE;
  if (!is_window_rect_full_screen(&rect, dpi))
    return FALSE;
  if (is_captured_by_system())
    return FALSE;
  if (NtGetTickCount() - thread_info->clipping_reset < 1000)
    return FALSE;
  if (!reset && clipping_cursor && thread_info->clipping_cursor)
    return FALSE; /* already clipping */
 ...
 */
BOOL WINAPI NtUserGetPointerInfoList(UINT32 id, POINTER_INPUT_TYPE type,
                                     UINT_PTR unk0, UINT_PTR unk1, SIZE_T size,
                                     UINT32 *entry_count, UINT32 *pointer_count,
                                     void *pointer_info) {
  FIXME("id %#x, type %#x, unk0 %#zx, unk1 %#zx, size %#zx, entry_count %p, "
        "pointer_count %p, pointer_info %p stub!\n",
        id, (int)type, (size_t)unk0, (size_t)unk1, (size_t)size, entry_count,
        pointer_count, pointer_info);
  RtlSetLastWin32Error(ERROR_CALL_NOT_IMPLEMENTED);
  return FALSE;
}
...
BOOL get_clip_cursor(RECT *rect, UINT dpi) {
  ...
  if (!status) {
    ...
    UINT raw_dpi,
        dpi_from = monitor_dpi_from_rect(*rect, get_thread_dpi(), &raw_dpi);
...
BOOL process_wine_clipcursor(HWND hwnd, UINT flags, BOOL reset) {
  ...
  if ((was_clipping = thread_info->clipping_cursor))
